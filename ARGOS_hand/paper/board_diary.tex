...
\section*{The granularity reversal does not exist: the worst frame is a tail, not a trend}

Fig.~2's caption currently reads the worst frame (mean weight $0.79$, $12.9\%$
improved) against the median frame ($0.46$, $82.8\%$) as a per-frame reversal
of the per-pixel weight--harm relation, ``which we report and do not
explain''. Measured over the full sequence instead of two frames, there is
nothing to explain: across $1{,}861$ frames with defined weight (of $2{,}114$
driven), per-frame mean weight correlates \emph{negatively} with per-frame
harm --- Pearson $-0.60$, Spearman $-0.68$ --- so a frame the head commits to
is a frame it is helping, exactly the benign direction the per-pixel AUROC
gives on development ($0.59$--$0.61$). The worst frame is a rare tail where
low support ($2.2\%$), high weight and harm coincide; it is included in the
figure by rank, and it is real, but it is not the rule. Support fraction
itself carries no signal at all (Spearman $-0.002$).

Two footnotes to the measurement. The caption's own numbers cross-check to
the third digit (median $0.459/82.8\%$, worst $0.791/12.9\%$), so figure and
series describe the same frames. And the harm scored on evaluation support
equals the harm scored on all GT-valid pixels to machine precision here,
because RAFT-Stereo leaves no GT-valid pixel invalid on this sequence.

Consequence for the paper, pending authorisation: the caption's ``granularity
reversal'' sentence should become the opposite statement --- the failure mode
is a rare high-weight, low-support frame against a background where weight
ranks frames correctly on development --- and the open caveat is that this
series is development-only, where the weight signal is known to exist; on
held-out data the per-pixel signal is chance, and the per-frame series there
has not been run.



\section*{A5 says the reverse pass is load-bearing after all, and the support contract goes categorical on the shipped head}

Two blockers close on measurement today, one against our hopes and one beyond
them.

\textbf{A5 (T2.5).} The head retrained with the forward--backward confidence
channel pinned to $1.0$ scores $5.31\%$ held-out Bad1 (macro) against the
shipped head's three-seed $5.84 \pm 0.22$ --- worse by $0.53$ points, beyond
the $0.37$ threshold, and D2 Bad1 drops from $12.7$ to $10.7$. So the
$13.1$\,ms reverse SEA-RAFT pass cannot be dropped for free, and the
limitation paragraph's inference-time substitution ($3.69 \to 3.74$--$3.79$,
inside seed spread) now has its counterpart: the trained head barely
\emph{reads} the cue at inference, but training \emph{with} it is worth half a
Bad1 point held out. The cue is a training-time scaffold. The efficiency claim
dies; what replaces it is more interesting, and the paragraph should say both
halves. Single seed, in the conservative direction (it argues for keeping the
pass we already run).

\textbf{Support contract (T2.4).} The unmasked twin of the shipped head,
evaluated on all $25$ DRENDS recording--backbone cells against the confined
head on the same $25$: unconfined, depth RMSE regresses in \emph{every} cell,
mean $+221.9\%$ (range $+29.5$ to $+594.5$); confined, it improves in every
cell, mean $-3.5\%$ ($-6.7$ to $-1.8$). The old $142$-channel version of this
result was unpaired ($14$ against $15$ cells) and gentler ($+110.4\%$); the
shipped-head version is paired, worse unmasked, and needs no transfer
argument. The paper's numbers should move to these.

Also today: A4 seeds 1 and 2 finished training (their evaluation is queued),
and the DRENDS matched-policy closure resumed after its job was killed
mid-flight --- one of five policies had completed.



\section*{The full-resolution context branch costs 22 milliseconds, not 3 percent}

T2.2 asked whether declining A3 on ``$16\times$ the context-branch compute''
survives being stated in absolute milliseconds, since the head is only
$1.00$\,ms and a reader could conclude we refused a better configuration to
save ${\sim}3\%$ of the module. Measured on the same H100 and cache grid as
every other runtime number: A3's module overhead is $51.08$\,ms against the
shipped head's $28.90$ and the $142$-channel base's ${\sim}33.5$. Stage-wise,
moving the context branch to full resolution takes evidence construction from
$5.13$ to $19.92$\,ms, alignment from $1.05$ to $3.62$, and the head from
$1.00$ to $1.31$. So the decline is defended by $+22$\,ms --- $77\%$ of the
entire shipped module --- not by $3\%$, and the sentence can stay with the
absolute number in it. Caveat for the twins: this measures A3 on its
$142$-channel base; A3b at $38$ channels will be cheaper in the evidence term,
and its cost argument should be restated from its own measurement when it
lands.



\section*{Convexity is what makes the EPE gain survive the split shift --- A4's own seeds say so}

A4's seeds 1 and 2 landed and the single-run reading survives with a sharper
shape. Over three seeds A4 wins the registered endpoint outright
($6.97\pm0.83$ held-out Bad1, largest in the table, against our
$5.84\pm0.22$) while its held-out EPE gain sits at $1.70\pm1.43$ against our
$5.61\pm0.42$. The new fact three seeds buy: on development the escape term
is \emph{ahead} of the convex head ($5.32$ against $4.13$ EPE), and the
development-to-held-out drop holds in all fifteen backbone--seed pairs, with
no training divergence in seeds 1 or 2 (validation oscillates in
$0.28$--$0.31$). So the unconstrained output buys in-domain accuracy that
does not transfer across the split, which is a measured argument for the
convex constraint that did not exist yesterday.

The line held: the withdrawn claim (convexity explains \emph{cross-backbone}
transfer) stays withdrawn --- the falsification clause fired and its
consequence is honoured verbatim. What the paper now states instead is the
narrower cross-\emph{split} claim, from A4's own three seeds. Paper updated
under authorisation: table row loses its dagger ($1.70\pm1.43$ /
$6.97\pm0.83$), the ablation passage rewritten, the Analysis sentence
updated from the single-seed $0.05\%$.



\section*{The out-of-domain closure: the denominator stands still and the numerator scales}

The matched-policy closure ran on DRENDS, the arena the freeze permits, and
Leonardo's reading of it is the right one, so it is the one recorded. Exact
pixel-pooled, tab-closure convention, RAFT-Stereo: best unlearned policy
$+0.63\%$ of EPE (fixed $w{=}0.5$; EMA3 $+0.37$, FB-confidence $+0.48$,
warped-raw $-0.07$, warped-recurrent $-0.51$) against TETHER's $+5.80\%$ ---
$9\times$ against $6\times$ on D2. The ratio widens through the numerator:
smoothing recovers the same small amount in both arenas, while the learned
gain tracks the raw error available ($0.276$\,px pooled raw EPE on D2 against
$1.286$ on the closure support --- not the $1.199$ of the BiDA common
support, a different array). That is the mapping section's condition measured
on a per-frame axis by an independent instrument.

Contribution (iii)'s universal fell for the second time and is now scoped:
on DRENDS the uniform blends recover $11\%$ of the gain (against $16\%$ on
D2) and none pays in bad pixels --- the trade is a property of the split as
much as of the policy. The contribution now says so, the closure section
carries the four-sentence out-of-domain paragraph in prose rather than a
table, and the conclusion's unscoped copy of the same claim was caught and
scoped in the same pass (the fifth-copy lesson, applied for once before
landing rather than after).



\section*{Leonardo's three corrections: the statistics we already had, the rule we broke, and the claim we withdrew too early}

Three edits from the author tonight, each verified before landing.

\textbf{A4 without the confession.} The post-hoc admission (``a reading chosen
after seeing the result'') was the one place a reviewer could say the endpoint
moved after the data. It was also unnecessary: A4's Bad1 margin ($+1.13$) sits
at $t=2.28$ against a critical $4.30$ at df${\approx}2$ --- not separable from
its own spread, which at $0.83$ is four times ours and twice the registered
threshold, whose comparable-spreads assumption it breaks. Its EPE loss is at
$t=4.5$ --- separable. The paragraph now declines A4 on the metric where the
difference is real, and the dev-to-held-out contrast ($5.32 \to 1.70$ against
our $4.13 \to 5.61$) becomes one juxtaposed line of reinforcement.

\textbf{The rule we broke ourselves.} ``Ahead on both endpoints'' against
full-res context read differences of $0.16$ and $0.37$ under a threshold of
$0.37$ --- the exact self-contradiction anyone with a calculator finds. Now:
indistinguishable by the rule the table is read under, separated by cost,
$28.9 \to 51.1$\,ms measured. T2.2 closed.

\textbf{The FB claim, resurrected properly.} A5 killed ``drop the pass'' at
training time; but the two facts together (training needs the cue, the trained
head barely reads it) point at train-with, deploy-without --- the shipped
checkpoint with $C^{FB}$ pinned at inference, which on D2 costs nothing. The
missing number is the same substitution on held-out D7. Declared before
running (declaration JSON beside the output: same checkpoint, Bad1/EPE within
$0.37$ of its own unpinned numbers = droppable at deployment), evaluation
launched on the existing no-FB module --- the one that produced the D2 number,
so the convention cannot drift.



\section*{Retraction: the 51 ms was contention, and the reviewer's fear was founded}

The diary entry of yesterday evening (``the full-resolution context branch
costs 22 ms, not 3\%'') is retracted. Leonardo asked how $51.1$\,ms of system
cost squares with a $1.00$\,ms head, and the answer is that it does not: in
the A3 timing run, the stages \emph{identical} between the two configurations
inflated fourfold (evidence $5.13 \to 19.92$\,ms, alignment $1.06 \to 3.62$)
while the only stage the variant actually changes moved $1.01 \to 1.31$. The
run was taken on a loaded node at 16:42; the baselines are called
\texttt{quiet} for a reason. The internally consistent reading is the
opposite of what I wrote: the full-resolution branch costs about $+0.3$\,ms
--- $16\times$ a ${\sim}0.02$\,ms branch, exactly the method section's
compute factor --- and the reviewer's suspicion that we declined a better
configuration to save ${\sim}3\%$ of the module was pointing the right way.

The paper now declines full-res on the registered rule alone
(indistinguishable in accuracy; the deviation buys nothing the rule can read
and spends $16\times$ the branch's compute), with no system-milliseconds
claim. A paired quiet re-measurement of both configurations is queued behind
the night's jobs; if it contradicts the $+0.3$\,ms reading, that lands here
too. Error bars added to the A4 trajectory line
($5.32 \to 1.70\pm1.43$ against $4.13 \to 5.61\pm0.42$).

Lesson filed next to the defaults: a measurement taken to settle an argument
inherits the argument's urgency, and urgency is how a contaminated number
gets into a paper the same day it is measured. The quiet-node discipline the
runtime baselines already encoded was the protocol; I stepped around it.



\section*{Addendum to the retraction: the +0.3 ms is a bound, not an estimate}

Leonardo's correction to my correction, and it holds: the $1.01$ in
``$1.01 \to 1.31$'' is the \emph{quiet} baseline while the $1.31$ is the
loaded-node run --- the same quiet-against-loaded comparison that produced
the $51.1$, at $1.3\times$ instead of $4\times$. Why load would inflate the
evidence builder $3.9\times$ and the head only $1.3\times$ is a question the
data cannot answer (memory-bound against compute-bound is a plausible shape,
not a demonstration), so part of the $+0.30$ may be load. \textbf{$+0.3$\,ms
is therefore an upper bound on the full-resolution branch's cost}, and the
paired quiet re-measurement decides the number. The paper needs neither: the
decline now rests entirely on the registered rule ($0.16$ points under a
$0.37$ threshold means there is nothing to decline --- in a tie, the
registered configuration stands), the $16\times$ removed from that sentence
where it invited the reviewer to compute ``they declined a configuration
over $1\%$'', and rephrased in III-D as a ratio on the branch
(``a compute choice on that branch, $16\times$ cheaper'') rather than a
justification.

Also this pass, with explicit approval: Fig.~1 loses panel (a) --- cropped
from the untouched asset via trim, reversible --- drops from double column to
single, and gains its first in-text reference, which it never had. The
submission build stands at exactly $10$ pages.



\section*{Two structural cuts, and the night queue holds three substitutions, not additions}

IV-C (the unified-evaluation metric family) is compressed to what the paper
uses --- prediction-independent support, non-gameable DTCE, the harm/benefit
masses, the aggregation convention --- with the full family moved to the
supplement's moved-material section. V-I (the BiDA head-to-head) loses forty
percent of its length and none of its numbers except two decorative ones. The
submission build stands at ten pages with the last about $85\%$ full, so
roughly $9.85$ effective from $10.4$ this morning; the next cuts on the list
are D4D, the meta-registration prose, and the intro--conclusion redundancy.

The night queue follows the rule that a job must have lines to land in: the
A5-with-real-cue converse (decides whether inference indifference to $C^{FB}$
is architectural, landing in the runtime paragraph either way), the per-frame
series on a held-out sequence (lands in the Fig.~2 caption's declared caveat,
six words either way), and nothing else. Twin seeds, extra backbones, and A7
are explicitly not queued: results without lines to land in are how a paper
grows back.



\section*{The held-out per-frame series: the sign survives, the strength does not}

On \texttt{dataset\_7\_keyframe\_4} ($1{,}919$ frames with defined weight, the
largest held-out sequence), per-frame mean weight against per-frame harm
gives Spearman $-0.26$ --- against $-0.68$ on the development sequence. Same
support definition as the D2 series and as Fig.~2 (gt-and-raw validity, not
the definitive D7 protocol; the two numbers must never be cross-quoted with
the tables). Support fraction carries a whisper of the opposite sign
($+0.09$).

The reading sits exactly between the two hypotheses the appendix caveat
allowed. The frame-level signal does not vanish held out (at $n{=}1{,}919$,
$-0.26$ is robustly nonzero), so ``the worst frame is a tail, not a trend''
survives the split; but two thirds of the correlation was development-only,
the same attenuation pattern as the per-pixel AUROC (benign $0.59$--$0.61$ on
D2, chance held out) at a coarser granularity that retains more. Proposed
caption clause, awaiting authorisation: ``(on a held-out sequence the
correlation weakens to $-0.26$ with the same sign)''. That closes the
declared caveat either way and adds the fourth independent instrument to the
selection-optimism thread: seed spread, per-pixel AUROC, the closure's
development margins, and now the per-frame series, each losing most but not
all of its development signal held out.



\section*{The declared endpoint holds: the reverse pass is droppable at deployment}

The shipped checkpoint with $C^{FB}$ pinned to $1.0$ at inference, on
held-out D7: Bad1 reduction $5.77$ against its own unpinned $5.69$
($+0.08$, inside even the $0.22$ seed spread, let alone the declared $0.37$
threshold); EPE $5.43$ against $5.49$. On D2 it reproduces the known
$3.74$ ($3.69$ unpinned). The declaration wrote the rule before the number
existed, and the number satisfies it: \textbf{train with the cue, deploy
without the reverse pass} --- $28.9 \to {\sim}15.8$\,ms, $45\%$ of the module,
the checkpoint unchanged. Together with A5 (training without the cue loses
$0.53$ points), the pair says the cue is a training-time scaffold in both
directions now, each measured held out. The converse arm (A5's head fed the
real cue) is minutes from landing and completes the argument that the
inference indifference is architectural.

\section*{A6: the twenty-five correlations are load-bearing}

Geometry-only (13 channels: disparities, residuals, motion, RGB --- no
quarter-resolution correlations) lands at held-out Bad1 $4.64$ against the
shipped $5.84\pm0.22$: $-1.20$ points, over three times the threshold, and
EPE $4.44$ against $5.61$. Removing two thirds of the evidence space costs a
fifth of the held-out gain. The $142 \to 78 \to 38$ ladder never tested below
$38$; now the floor is measured, and the $25$ upsampled correlations are what
the $38$-channel head cannot do without. Both results are diary-only until
morning authorisation; the runtime paragraph rewrite (withdrawal $\to$
deploy-without claim) and the A6 table row are drafted and waiting.



\section*{T3.3: the missing third of the frames are exact Bad1 ties}

The per-frame buckets ($56/14$ held out, $45/18$ development) are strict
inequalities on per-frame Bad1 with no deadband; the remainder is $29.7\%$
(D7) and $36.5\%$ (D2) of frames where refined and raw Bad1 are exactly
equal. These are the cleaner-than-average frames --- median raw Bad1 among
ties $3.9\%$ against $7.6\%$ overall on D7 --- where the blend's
sub-threshold corrections flip no pixel across the $1$\,px line: consistent
with, not contradicting, the protocol note that the blend moves almost every
pixel on the support. A further $1{,}895$ D7 frames sit at exactly zero Bad1
on either side and are excluded by the plot's log axes, which the manifest
records. Proposed half-line for Sec.~V-G, awaiting authorisation: ``the
remaining third are exact Bad1 ties --- cleaner frames whose sub-threshold
corrections flip no pixel across the threshold''.



\section*{The converse arm closes the FB argument, and A3b turns the tie into a win}

A5's head fed the real forward--backward confidence it never trained with:
held-out Bad1 $5.29$ against its pinned $5.31$ ($-0.02$), EPE $4.89$ against
$5.05$. Symmetric with the shipped head's $+0.08$ under pinning. Neither head
reads the channel at inference regardless of what training showed it, so the
inference indifference is a property of the architecture, not of a
checkpoint: the cue is a training-time scaffold in both directions, each arm
measured held out. The deploy-without claim is no longer anecdotal.

A3b --- the full-resolution context twin on the shipped 38-channel base ---
lands at held-out Bad1 $5.08$ against the shipped $5.84\pm0.22$: $-0.76$,
twice the threshold, one seed. On the 142-channel base the same deviation was
indistinguishable from us; on our own base it is worse. The ablation
passage's ``the registered configuration stands'' upgrades from a tie-break
to a measured preference, and the quarter-resolution context is load-bearing
on the head we ship. A4b lands within the hour.



\section*{The night closes: the quiet pair confirms the bound, and A4b overfits development harder than its parent}

\textbf{Quiet paired runtime.} On the empty node, back to back: shipped
$29.72$\,ms (evidence $0.71$, head $1.05$), A3 $35.65$\,ms (evidence $5.36$,
head $1.35$). The full-resolution context branch costs exactly the
$+0.30$\,ms the bound predicted; the rest of the gap is the $142$-channel
evidence term, and the contaminated $51.1$ is now formally replaced. The
$16\times$ ratio in III-D describes a $+0.3$\,ms branch, which is why it
stays a ratio and not a justification.

\textbf{A4b.} The unconstrained-output twin on the shipped base, one seed, no
training divergence (validation minimum at epoch $7$, best-validation
selection): D2 Bad1 reduction $27.5$ against the shipped $12.8$ and D2 EPE
$11.3$ against $3.7$ --- enormous in-domain --- collapsing to $7.3$ and
$4.4$ held out. The same shape as its $142$-channel parent
($5.32 \to 1.70$) but steeper from higher: the escape term's development
gain is $2.5\times$ its held-out gain where ours \emph{rises} across the
split. Second independent instance of the cross-split argument for
convexity, on the base we ship. Both twins ran clean; T2.1's measured
answers are now complete --- full-res loses $0.76$ points held out, and the
escape hatch overfits development on either base.

Everything queued last night has landed and every result is in this diary.
The paper touches wait for morning authorisation: the runtime paragraph
(deploy-without, both arms), the ablation prose sentence for the twins, the
A6 row, the D7 caption clause, the V-G tie half-line, and the raw-EPE
reconciliation clause.



\section*{The morning pass: the two-base finding becomes the section's spine}

All six edits landed under authorisation, with Leonardo's upgrade of the
third: A3b is not a footnote but the inversion that explains the section ---
against the $142$-channel base none of the registered deviations is
load-bearing, against the shipped head two of them are ($-0.76$ full-res,
$-1.20$ geometry-only), and the learned evidence was absorbing the
difference. The full-res decline is now one line on the right base. A4b
enters in the section's existing register with both facts stated: it wins
the registered endpoint on the shipped base too ($7.32$, one seed) and loses
EPE ($4.41$ against $5.61$), its development margin not surviving the split.
One correction to the review that requested this: the $7.3$ is Bad1, not
EPE --- A4b sits below us on held-out EPE, above us on held-out Bad1,
which is its parent's exact pattern.

The runtime paragraph now states deploy-without with all three arms, each
declared single-seed. A5 and A6 join the table under the shipped base
(indentation is the base marker; caption rewritten for two bases); the
Fig.~2 caption carries the $-0.26$ held-out clause; V-G names the exact-tie
third; the closure paragraph reconciles the three out-of-domain raw EPEs as
three supports. T2.1, T2.3 and T3.3 close; T3.1 and T3.2 remain, plus the
structural cuts (D4D, meta-registration, intro--conclusion) toward eight
pages. Submission build: ten pages, the last now full.



\section*{The author's compression pass: nine pages, appendix retired, two pointers backed}

Leonardo's editorial pass lands: abstract, intro, related work, captions and
the BiDA/TC-Stereo section compressed; the blocker appendix and its gate
retired now that all tiers are clear (the cleared log lives in this diary and
in git); bibliography to scriptsize; the A5 table row and the A4b sentences
replaced by supplement pointers. Three companion fixes from this side: the
figure path pointed at a directory that exists only on the author's machine
and is restored to the tracked asset; and the two new ``(supplement)''
pointers were dangling --- the A5 training cost ($5.31$ against
$5.84\pm0.22$, $0.53$ points) and the A4b twin numbers ($11.25 \to 4.41$ EPE,
$27.49 \to 7.32$ Bad1) now have their backing paragraphs in the supplement's
moved-material section. Both documents compile clean with zero undefined
references. The main paper stands at nine pages against eight; the author's
own header lists the next cuts and marks the DTCE-column drop from Table~I as
the mandatory one.
...
The six prose cuts landed and the float effect ran in reverse, exactly as
Leonardo predicted it might: removing 49 lines of text let the floats repack
and returned more than the arithmetic promised. The paper compiles at
\textbf{eight pages flat}, bibliography complete, zero undefined references,
with none of the reserves spent --- the map-sweep table keeps its grid, the
bibliography keeps its entries, Fig.~2 keeps its worst frame, and no
negative \texttt{vspace} was used.

What went, in this final pass: V-I compressed to its results (protocols and
asymmetries to the supplement, Vid13 kept); the AUROC limit halved around
the benign-direction reading; the related-work dataset paragraph reduced to
role assignments; the limitations opener to the two caveats that live
nowhere else; the map-control paragraph to one sentence under its own table;
and the channel enumeration delegated to Fig.~1's right panel. Earlier in
the day: Table~I dropped its development block and DTCE columns to the
supplement and became single-column --- the confirmation arenas in table
form, the paper's own epistemology --- and the prose adopted one declared
aggregation.

The paper stands at the limit it was asked to meet, scientifically closed,
with every number traceable to a run and every retraction in this diary.



\section*{The deploy-without claim gets its three seeds, and the delta is zero}

The pinned-at-inference substitution evaluated on shipped seeds 1 and 2
(evaluation only; the checkpoints existed): within-checkpoint held-out
deltas of $+0.08$, $-0.04$ and $+0.03$ Bad1 --- $+0.02\pm0.06$ over the
three seeds, crossing zero --- and $-0.09\pm0.06$ EPE, marginally
\emph{better} pinned. The deployment claim no longer rests on one seed:
dropping the reverse pass costs nothing measurable on any of the three
checkpoints, held out. Proposed one-line upgrade to the runtime paragraph
(awaiting authorisation): quote the three-seed delta and demote the
single-seed caveat to the training arm alone, which is where it still
belongs.

Training queue: A3b both seeds and A6 seed 1 done; A6 seed 2 and A1 seed 2
finish tonight and the watcher evaluates all six.



\section*{Retraction: the two-base finding dissolves at three seeds, and the section is stronger for it}

The complete seed grid landed and reversed the two-day-old finding. On the
registered endpoint no deviation of the shipped head separates: A5
$5.42\pm0.42$, A6 $5.36\pm0.68$, full-resolution $5.57\pm0.68$ against
$5.84\pm0.22$, every spread exceeding the $0.37$ threshold it would be
judged by. ``Two of the same decisions are load-bearing on the base we
ship'' is retracted; what replaces it, under authorisation, is the
threshold-premise reading (the rule is read, not mechanically applied, where
the spreads outgrow it), the EPE asymmetry (direction consistent everywhere,
A6 at under four standard errors: the evidence space shows on the mean where
it does not show on the registered endpoint), and the seed-$0$ clause tying
this to the closure and the base --- the third appearance of one pattern.
The FB paragraph now carries the three-seed pinned delta ($+0.02\pm0.06$)
and softens the training arm to ``trends lower without separating''; the
deploy-without claim is untouched, resting as it does on within-checkpoint
pinning. Table~IV is three seeds on every row; A1's seeds confirm its
single-run reading exactly (the only variant whose first seed was not
optimistic). Eight pages, all documents compiling clean.

The transferable lesson, recorded as the author phrased it: of five variants
given seeds, three changed reading from one seed to three, and in every case
the single seed was optimistic. That is not symmetric noise --- it is
selection, because seed $0$ is the seed everything else was built on. A
number computed on the seed that survived development inherits development's
optimism, whatever the number measures.



\section*{The review package lands: self-contained eight pages, every number verified first}

Three simulated ICRA reviews (3/2/3) converged on verifiability, and the
submission cannot carry a supplement, so the paper became self-contained.
Every number added was verified against the runs before writing: absolute
depth (held-out MAE $2.79\to2.71$\,mm, Bad-2mm $32.5\to31.8\%$; DRENDS
$14.7\to14.1$\,mm --- failed geometry improved, not good geometry polished),
the five per-backbone D2 reductions ($1.41/2.28/8.19/3.10/2.24$), DTCE
ranges, the OOD blend Bad1 costs ($+0.42/+0.53/+0.45$), TC-Stereo's temporal
path from the run of record ($0.622$, $10.37\%$ --- the main text's claim
was always backed; it was the supplement that was stale), the $144\times180$
grid against native $1280\times1024$, the normalisation constants and width
$48$, and the DTCE formula restored to IV-C.

Reviewer-3 arithmetic fixes, each checked: the held-out headline now
declares both conventions ($5.49$ pooled / $5.40$ per cell); the DRENDS
frame count is $7476$ \emph{in total}, not each (the 'each' inflated the
arena fivefold); the $H{=}1$ rejection is $1.20$ points, not $1.24$, and
$H{=}2$ ($3.02$) is no longer silently dropped from the spread; the map
keeps three justifying criteria after V-F's own disqualification of the
fourth; the abstract names the term as consistent-with rather than
identified; the mean state age is $(H{+}1)/2$; the unsupported bootstrap
sentence is deleted (the only bootstrap on disk is the 142-channel one);
and the blanket 'no registered decision fires' now carves out the one that
did (A3 on its own base, declined on compute).

Space: the map-sweep grid became six lines of prose carrying all twelve
numbers, two peripheral references left the bibliography, the V-J
redundancy created by the carve-out was folded, and captions gave up their
last slack. Eight pages, no overfull, no undefined references, and an
availability line promising the extended version, code, checkpoint and
curation release --- which commits us to actually bonifying the stale
supplement into that extended version.



\section*{The conversion survives its audit, and page 1 stops re-asserting a withdrawn claim}

Leonardo asked which raw the new native-unit conversion converts, on the
suspicion that a raw from one pool had been multiplied against a refined from
another. The audit says no. Both figures come from one file, one method row,
one aggregation: \texttt{experimental\_closure/d2\_A2\_no\_learned\_evidence/}
\texttt{summary.csv}, \texttt{canonical\_h4}, fifteen backbone--sequence cells
macro-pooled --- raw $0.276202$, refined $0.265998$, reduction $3.6943\%$,
which is the $3.69$ of the closure table. The raw is byte-identical to the
$142$-channel base's file, as it must be: same alignment, same support
contract, only the head changes. Native $1.96 \to 1.89$ stands.

The near-miss is worth recording because it was one directory away. The base
closure's \texttt{canonical\_h4} refines to $0.265028$ ($4.05\%$) --- the arm
with learned evidence, the one Sec.~V-J calls a seed-$0$ artefact. Converting
\emph{that} would have published $1.88$: a defensible-looking number
describing a head we do not ship.

The second correction is structural. Contribution (ii) enumerated the head's
four parts under a heading claiming they make cross-backbone transfer
possible --- and one of those parts is the convex action space, whose
cross-backbone claim A4's pre-registered falsification clause already
withdrew, six pages later, in this same paper. The list also repeated the
abstract almost verbatim on the same page. It is gone. In its place the
contribution says which parts the evidence actually holds up (bounded
recurrence, the support contract) and states the withdrawal itself on page 1
rather than letting the enumeration quietly reinstate it. Eight pages, no
overfull, no undefined references.



\section*{The specialist control is launched, and its handicap is ours to declare}

Reviewer 2's single most important missing ablation is the one contribution
(i) invites: if one checkpoint serves every backbone, what does a checkpoint
trained \emph{for} a backbone do on that backbone? Five arms are training,
one per cached SCARED-C estimator --- the three seen and, deliberately, the
two the paper calls unseen, because those are the cells where the specialist
prices the word.

The design decision worth recording is the handicap, and it runs the other
way from the one a reader expects. Twelve epochs over three backbones is
thirty-six backbone-epochs of gradient steps; twelve epochs over one backbone
would be a third of that, and a specialist beaten under a third of the budget
would have proved nothing. So the specialists get $36$ epochs, three times
the selection points, and checkpoint selection on their own backbone's
validation rather than a mean over three. The generalist gets none of it.
This is written into the declaration before any arm launched, together with
its consequence: a specialist win of ordinary size is not evidence against
the shared module, while a specialist loss is. Single seed, so the $0.37$
threshold reads as a bound and not a measurement.

Mechanically the control is a one-name patch --- \texttt{SEEN\_BACKBONES}
rebound in the runner's namespace, the same technique the ablations use so
that nothing on disk changes and the canonical hash guard keeps meaning what
it says. The name is read twice by the runner, once for the datasets and once
for \texttt{split\_audit.json}, which is why the test asserts both: a patch
reaching only the first would train on one backbone and record three, the
provenance lie the shared \texttt{SHIPPED\_BASE\_VARIANTS} tuple was
introduced to prevent.



\section*{The seed's rank is a property of the arena, and the sentence we nearly wrote was false}

A reviewer objected that Table~I rests on seed $0$ while the paper itself
demonstrates seed $0$ is an outlier. The obvious repair was a caption clause
saying seed $0$ understates. Leonardo stopped it on the ground that nobody had
checked, and that Sec.~V-J already shows the seed's rank moving with the
configuration --- low tail for the shipped head, high tail for the
$142$-channel base --- so a rank measured on D2 is not evidence about D7 or
DRENDS.

It moves with the arena as well. Pooled over backbones, seed $0$ is the lowest
of three on D2 EPE ($3.69$ against $4.40$ and $4.31$) and on D7 Bad1, middling
on D7 EPE and D2 Bad1, and \emph{the highest of the three} on both DRENDS
metrics ($5.37$ against $5.27$, $5.14$; $5.57$ against $5.45$, $5.14$). DRENDS
is half of Table~I. The clause would have been a falsehood in the caption of
the main table, defended by nothing but its own plausibility.

The defence that does not depend on the rank survives: every one of the $13$
cells measured at three seeds improves at all three, on EPE and on Bad1.
Thirteen rather than fifteen because DRENDS carries three seeds on three
backbones; the two missing evaluations are running, which will take Table~I's
own confirmation from eight of ten cells to ten. The sign of each cell, not
the size of seed $0$, is what contribution~(i) actually claims, and it is
seed-invariant on everything measured.

Two things fell out of the same recomputation. The seed study sitting in
\texttt{results/seed\_variance} is the $142$-channel base, not the head we
ship; recomputed on A2 it reproduces all four of Sec.~V-D's published figures
to the decimal, which is the check that the new script is reading what the
paper reads. And Sec.~V-A's named exception --- S$^2$M$^2$-S on keyframe~$2$,
worse by $0.28\%$ --- is itself a seed-$0$ artefact: the same cell improves by
$1.39$ and $1.25$ at seeds $1$ and $2$. We have been quoting a seed as a
property of a sequence.



\section*{The map condition does not hold between backbones, and the pose floor does not order the cells}

Two questions closed on data already on disk. The first is the loop's own
fourth item: does ``it helps where the raw prediction is poor'' survive being
asked of backbones rather than of splits? It does not. Across all $35$
world-frame cells the rank correlation between raw excess and the reduction is
$+0.72$, but D2's excess spans $0.171$--$0.290$\,mm and D7's $0.518$--$1.113$,
two clouds that never touch, so that figure measures the separation between
the splits and not a relation inside either. Within D2 the correlation is
$-0.47$; within D7 it is $+0.11$; per sequence, across the five backbones, it
scatters from $-0.30$ to $+0.60$ with no stable sign. Sec.~V-K already declines
to claim the mechanism and already says the ordering fails inside D2. It does
not say the same of D7, and now that is measured.

The second is the objection a reviewer raised about pose noise, and half of it
needed no compute at all. The ground-truth cloud is one keyframe's structured
light moved by these very poses, so its world-frame spread \emph{is} the pose
residual plus resampling --- and it is identical across all five backbones
within a sequence, which is what a pose-side quantity should look like.
Against it the refinement effect is $0.53$ of the floor on D2 and $0.89$ on
D7: the reviewer is right that this is not comfortably above the noise, and we
had never said so.

But the floor does not explain the finding. As a predictor of the reduction
over $35$ cells it ranks at $+0.08$ against raw excess's $+0.72$: if the map
result were a pose artefact, the floor would be the variable that orders the
cells, and it is not. The sharper version is a pair. D2 keyframe~$4$ has the
lowest floor of all seven sequences, $0.0387$\,mm, and sits at $-6.8$ to
$-16.1\%$; D7 keyframe~$3$, at $0.0584$\,mm, sits at $+10.9$ to $+17.8$. The
sign does not follow the floor. The perturbation sweep is queued to settle it
properly --- independent per-frame rigid jitter, the same perturbed poses on
ground truth, raw and refined, since the metric is an excess over the floor
and drawing separate noise per cloud would model a situation that never
occurs.



\section*{Withdrawing a predictor is shorter than hedging it}

The three edits went in, but not the third in the form proposed. It had been
written as a line strengthening a caveat that already existed. Leonardo
refused that framing on the ground the numbers do not permit a caveat: $\rho$
of $-0.47$ inside D2 and $+0.11$ inside D7 means raw excess orders the cells in
neither split, and the $+0.72$ across all $35$ exists only because the two
clouds never touch. Adding those figures as reinforcement would leave standing
an explanation they dismantle, and a reviewer who reads them asks why the
conclusion was not drawn.

So the account is withdrawn rather than qualified, and what replaces it is the
number that actively defends the result: the ground-truth cloud is the
keyframe geometry carried by these same poses, so its spread measures their
residual, and it ranks the cells at $+0.08$ against raw excess's $+0.72$. If
the map reversal were a pose artefact, the floor would be the variable that
orders the cells. It is not.

The lesson is the one about scope. A withdrawal cannot live in one paragraph:
the same claim was asserted in five places --- V-H naming a condition, the
closure's cross-reference calling it the deciding quantity, the metrics
paragraph resting the robotics claim on it, the abstract naming a term, the
conclusion naming it again. Retracting only where the evidence was discussed
would have left the paper contradicting itself in its own abstract. All five
moved together.

And the withdrawal is shorter than the hedge was. Eight pages held after
compressing six paragraphs that said their point twice and tightening the
bibliography's leading; no fact left the paper. The two seed edits landed
alongside: the sign is what contribution~(i) claims and it is seed-invariant,
and keyframe~$2$'s exception keeps its place while gaining the two seeds where
it reverses.



\section*{Three reviewer objections, and the one where the proposed fix was backwards}

The architecture objection was the only one of the four with no rhetorical exit.
Without a supplement, ``the full channel specification is in the extended
version'' points at a document the reviewer does not hold, so the paper was
literally unreproducible from itself. The specification is now in: the $25$
quarter-resolution correlations with their window, dilation and bound, the $13$
grid channels with every normalisation constant, and the head in layer form.
Both readings were checked against invariants rather than trusted --- the
channels sum to $25{+}13=38$ and the layer shapes to $154{,}874$ parameters,
the two figures the paper already publishes, and a misread architecture breaks
both.

Leonardo then asked whether Fig.~1's channel panel was worth its space. It was
not, and the reason is that our own edit had made it redundant: one
five-segment bar, $16/9/4/6/3$, showing six numbers that III-C now states
exactly and with their normalisation, while costing a full-width banner across
both columns. His re-authored single panel is $252\times90.96$\,pt, exactly
one column, so it renders at natural size where a trim of the old asset would
have given $89\%$. The paper went from zero page slack to twelve or fifteen
lines, and the first thing bought back was not prose but two accuracy losses
from the earlier compression: Fig.~2's correlation is again over the frames
\emph{with defined weight} rather than all of them, and Related Work again says
that a lower mean error can hide newly created failures.

The third objection is the one worth recording properly. ``No published method
is simultaneously causal, black-box and estimator-agnostic'' is a universal
claim over a literature, falsified by one counterexample, and the adjacent line
it ignored was task-agnostic video temporal consistency. Leonardo proposed
naming that line and separating it on causality: it optimises over a clip
rather than deciding causally per frame.

That differentiator is backwards. Lai et al. is a recurrent network whose
output frame is fed back as input at the next step --- causal at frame level,
on our own recurrence structure. Writing the opposite would have been falsified
by opening the cited paper, which is a worse failure than the negation it
replaced. Two smaller things came with it: the citation had been dropped from
our bibliography in an earlier page recovery, so it would have rendered as a
missing reference, and the ``weights no longer retrievable'' line he
remembered as being about Lai is about Neural Disparity Refinement.

What replaces it is better than either version. That line regresses a
stabilised frame where we emit one scalar per pixel confined to an interval,
and it optimises consistency --- the objective Sec.~VII shows a degenerate
replay wins outright. Its recurrence carries no re-anchor, and Table~II already
measures what removing ours costs here, $19.17\%$ of EPE. The comparison is now
made with two of our own numbers instead of with a claim about what exists, and
the only surviving negation is bounded to what was checked: no causal,
black-box, estimator-agnostic method \emph{for stereo disparity} that we could
locate.



\section*{The map reversal survives the poses being wrong}

The sweep ran. Seven RAFT-Stereo cells, five paired magnitudes, five draws
each, the same perturbed poses on ground truth, raw and refined. The zero-noise
column of every run reproduces the published number to $10^{-9}$\,mm, which is
the check that mattered: the refactor that made the sweep nearly free is
exactly the kind that changes a number in silence.

At $\sigma_t=0.1$\,mm and $\sigma_r=0.057^\circ$ --- enough to double or
triple the observed floor of $0.039$--$0.214$\,mm --- all seven cells keep
their sign and the reduction moves by at most $3.4$ points. Signs still hold at
$0.5$\,mm. And the noise does not manufacture the split: it widens it, D2
going more negative and D7 more positive. An artefact would blur the
difference; an amplified real effect exaggerates it.

The failure mode at $1.0$\,mm is worth recording because it is not the one we
were looking for. The sign does not flip --- the denominator dies. The
perturbed reference cloud reaches $1.03$\,mm on D2 keyframe~$2$ while raw
reaches $1.04$, so the excess the reduction is a ratio \emph{of} has collapsed
from $0.224$ to $0.017$\,mm, and one draw returns $+92\%$: noise over noise.
That is a bound on what the test can say, not on what the result is, and it
goes in the reporting rather than being quietly dropped.

Together with the floor's own ranking --- $+0.08$ against raw excess's $+0.72$
over the $35$ cells --- the pose objection is now answered twice, from
independent directions: the floor does not order the cells, and moving the
poses does not reorder them either.



\section*{The control we asked for answers 2--2, and that is the answer}

Five specialists, one per backbone, each the shipped head trained on that
backbone alone. On D7 Bad1 --- the endpoint registered before any arm
trained --- the generalist wins Fast-FoundationStereo by $0.71$ and RAFT-Stereo
by $0.60$, the specialist wins CREStereo by $0.42$ and S$^2$M$^2$-S by $0.63$,
and StereoAnywhere at $+0.05$ is inside the spread. Two to two with a tie. On
the endpoint we chose in advance, the shared module does not beat modules
trained per backbone.

Held-out EPE runs the other way: the generalist ahead on three arms, behind on
none, RAFT-Stereo by $2.73$ points. The disagreement between the two metrics is
the finding rather than noise around it. The shared module wins mean error; the
specialists, where they win at all, win bad pixels.

The declaration written before any of this says a specialist win of ordinary
size is not evidence against the shared module, because every deviation
favoured the specialist: matched gradient budget, three times the selection
points, validation on its own backbone instead of a mean over three. That rule
applies here and it is exactly why the asymmetry was declared in advance. It is
not a licence to soften the number. Two arms beat us on the registered endpoint
and the paper will say so; the rule explains why that does not overturn
contribution~(i) --- which claims one module improves every cell, not that it
beats a per-backbone module --- and not why the result should be reported
smaller than it is.

Two things we do not understand and are not going to invent an account for
tonight. Three of five specialists collapse on D2 Bad1 --- $10.61\to4.78$,
$9.95\to3.33$, $10.71\to7.54$ --- on the very split they selected their own
checkpoints on, and two of those three then tie or win D7 Bad1. And every arm
selected at epoch $34$--$35$ of $36$, at the edge of the budget, so we cannot
say the specialists had finished improving. That last one is the reviewer's
obvious question about any arm that lost, and it belongs in the comparison
rather than in a rebuttal.



\section*{The same numbers, read on their own scale}

Re-reading the specialist grid in daylight changed what it says, and the change
is a correction to last night's entry rather than an addition to it.

Last night the control was read in relative reductions, which is the paper's
convention: two arms to the generalist, two to the specialist, one tie, and the
conclusion that sharing sometimes costs. In absolute terms those same five gaps
are $0.003$, $0.026$, $0.032$, $0.040$ and $0.042$ percentage points of
bad-pixel rate, against a raw rate near $6\%$. A two-to-two split among
differences of four parts in ten thousand, running in both directions, is
backbone idiosyncrasy. It is not a price.

The mistake has a shape worth naming, because it is the same one the paper
warns about elsewhere: a ratio with a small denominator turns a tenth of a
point into a headline. On D2, where raw Bad1 is $1.4$--$2.2\%$, it does worse.
The ``collapse'' recorded last night --- $9.95\to3.33$ for
Fast-FoundationStereo --- is $1.445\%\to1.551\%$ of pixels. The sentence drawn
from it, that single-backbone training produces a worse model rather than one
that generalises worse, does not survive its own magnitudes and is withdrawn.

What does survive is cleaner than what we thought we had. On bad pixels the two
are the same model to within the measurement. On mean EPE the generalist is
ahead on four backbones of five, RAFT-Stereo by $0.013$\,px, and behind on one
by $0.002$. An advantaged specialist --- matched budget, three times the
selection points, its own validation --- is not better than one shared
checkpoint. That is what went into V-J, with one seed and the epoch-$34$--$35$
selection stated in the same sentence, because both are the first thing a
reviewer will ask about any arm that lost.
