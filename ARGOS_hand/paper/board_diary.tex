...
\title{
    \textbf{ARGOS Temporal Stereo Refinement}\\
    \large From Universal Refinement to Lightweight Domain Adaptation
}
\author{}
\date{\today}
...
it, none exceeds $0.005$. In domain it costs $0.69$ percentage points of
Bad1 reduction on held-out D7 --- more than a hundred times the largest
DRENDS margin.

So the learned stereo evidence buys in-domain accuracy and buys nothing
out of domain. That is worth reporting, and it is a smaller claim than the
one we retracted this morning, which had A2 winning out of domain by
threefold. The honest version is \emph{no worse}, not \emph{better}, and
the difference between those two sentences is the whole reason the
morning went the way it did.
...
from the runtime, costs $0.69$ percentage points of Bad1 reduction in
domain, and is indistinguishable out of it. That is the third and final
version of this result today: first threefold better, then detectably
better, now neither. Each revision came from measuring something that had
been assumed, and each made the claim smaller.
