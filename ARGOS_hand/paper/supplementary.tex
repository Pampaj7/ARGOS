...
$H$ & EPE & $B^+/H^+$ & NewBad1 (\%) \\
\midrule
1 & 0.269001 & 2.39 & 0.0361 \\
2 & 0.267265 & 2.50 & 0.0381 \\
4 & 0.265028 & \textbf{2.53} & 0.0467 \\
6 & \textbf{0.264656} & 2.30 & 0.0676 \\
8 & 0.264912 & 2.04 & 0.1282 \\
\bottomrule
\end{tabular}
\end{table}
...
by $0.14\%$ over $H=4$. Over the same step the beneficial-to-harmful
magnitude ratio falls from $2.53$ to $2.30$ and then to $2.04$, while
newly introduced Bad1 pixels rise from $0.0467\%$ to $0.0676\%$ to
$0.1282\%$ --- nearly tripling between $H=4$ and $H=8$. Letting the
...
The correction is real but small. Only $6.2\%$ of D2 pixels have the
...
$37.2\%$ to $34.5\%$. We report it because the published figure is
...
$34.5\%$ of the continuous oracle its convex action space could reach;
only $6.2\%$ of D2 pixels have the ground truth bracketed by the two
...
$2.10$ and $3.30$ there? The two statements are about different things,
...
\end{equation}
%
Pooled over the five backbones this evaluates to $2.839=1.642\times1.728$
on held-out D7 and $2.662=1.355\times1.965$ on D2. On D7, $62.2\%$ of
interventions improve the pixel and the improvements are $1.73\times$
larger than the regressions ($0.0594$ against $0.0344$\,px).
...
\emph{which} pixels fall in the harmful $37.8\%$, and on held-out data it
...
directly comparable. Canonical is the mean of three seeds; every variant
...
A2 & 3.69 & 12.84 & 19.67 & 13.61 & 5.49 & 5.69 & 4.85 & 5.92 \\
A3 & 4.28 & 9.22 & 19.28 & 13.76 & 5.61 & 5.85 & 5.83 & 6.09 \\
...
Bad1 ($8.81$ against $8.40$) but clearly worse on EPE ($4.18$ against
$3.55$); on D7 it is marginally better on all four. This is the same
...
\paragraph{What we are not doing.} A2 is the strongest variant and it is
not being promoted to canonical on the strength of a held-out win, because
choosing on the held-out split is what makes a held-out split stop being
one. It is being given the same three-seed treatment the canonical model
has, and the comparison that could promote it is on D2, the development
split, which exists for exactly that purpose.
