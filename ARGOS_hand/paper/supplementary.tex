...
gives the relative change after refinement on the shipped head. The previous
version of this table did not match either configuration's stored results and
has been recomputed from the run of record.
...
D2 & S$^2$M$^2$-S          &  -3.0\% &  +8.6\% &  +3.1\% &  +8.0\% \\
D2 & RAFT-Stereo           &  -6.8\% &  +6.1\% &  +1.1\% &  +7.1\% \\
D2 & Stereo Anywhere       & -40.3\% & -29.6\% & -28.6\% & -21.1\% \\
D2 & CREStereo             & -10.0\% &  +1.7\% &  -2.0\% &  +2.5\% \\
D2 & Fast-FoundationStereo &  -1.6\% & +10.7\% &  +3.6\% &  +9.6\% \\
D7 & S$^2$M$^2$-S          & -21.4\% & -13.5\% & -10.4\% &  -7.0\% \\
D7 & RAFT-Stereo           & -33.4\% & -21.1\% & -14.1\% & -11.9\% \\
D7 & Stereo Anywhere       & -36.4\% & -25.2\% & -20.6\% & -15.3\% \\
D7 & CREStereo             & -27.7\% & -16.2\% & -11.3\% &  -8.9\% \\
D7 & Fast-FoundationStereo & -19.5\% &  -9.3\% &  -6.7\% &  -5.1\% \\
\midrule
\multicolumn{6}{l}{\emph{OPW}}\\
D2 & S$^2$M$^2$-S          & -14.6\% &  -5.8\% &  -2.7\% &  -1.7\% \\
D2 & RAFT-Stereo           & -16.9\% &  -7.0\% &  -3.3\% &  -2.0\% \\
D2 & Stereo Anywhere       & -41.1\% & -25.7\% & -14.8\% &  -8.6\% \\
D2 & CREStereo             & -18.9\% &  -8.7\% &  -4.1\% &  -2.8\% \\
D2 & Fast-FoundationStereo & -14.9\% &  -6.3\% &  -3.1\% &  -1.9\% \\
D7 & S$^2$M$^2$-S          & -27.3\% & -15.7\% &  -8.4\% &  -4.3\% \\
D7 & RAFT-Stereo           & -38.2\% & -22.7\% & -12.1\% &  -8.4\% \\
D7 & Stereo Anywhere       & -42.1\% & -26.8\% & -17.0\% & -10.1\% \\
D7 & CREStereo             & -33.6\% & -18.6\% &  -9.6\% &  -5.7\% \\
D7 & Fast-FoundationStereo & -25.8\% & -12.4\% &  -5.8\% &  -3.1\% \\
\bottomrule
\end{tabular}
\end{table}

On held-out D7 both improve in all $20/20$ backbone--horizon cases. On
development D2 they disagree, but not uniformly, and the shape of the
disagreement is worth stating precisely: OPW improves in all $20$ cells, while
$\TEPE_{\mathrm{corr}}$ improves in $9$ of $20$ --- at $k{=}1$ for every
backbone, and at every lag for Stereo Anywhere, the backbone whose raw
prediction is weakest. The $11$ cells where it degrades are all at $k\geq2$ on
the four stronger backbones.

This rules out a convenient explanation. One might attribute a rising
temporal-error metric to a grid metric penalising legitimate scene motion, but
$\TEPE_{\mathrm{corr}}$ \emph{is} motion compensated and still degrades at the
longer lags on D2. The two ask different questions: OPW asks whether
...
self-consistent while, beyond one frame, tracking the true change less
faithfully. We report this against our own interest.
...
The main paper is eight pages. The objects that were in it are now here in
full, and nothing measured was deleted to make room.

\paragraph{The development block and DTCE columns of the main grid.}
Table~I of the main paper shows the two confirmation arenas; the development
block and the change-error columns it dropped for space are here, same
protocol, same seed-$0$ checkpoint, cells pooled per backbone.

\begin{table}[htb]
\centering
\small
\caption{D2 development cells and DTCE (macro reduction in change error at
$k{=}4$, higher is better) for the five backbones of main-paper Table~I.}
\begin{tabular}{ll|cc|cc}
\toprule
& & \multicolumn{2}{c|}{D2 development} & \multicolumn{2}{c}{DTCE} \\
\cmidrule(lr){3-4}\cmidrule(lr){5-6}
Backbone & Status & EPE & Bad1 (\%) & D7 & DRENDS \\
\midrule
S$^2$M$^2$-S & seen & $.2704\rightarrow\mathbf{.2658}$ & $1.39\rightarrow\mathbf{1.27}$ & $6.5$ & $11.3$ \\
RAFT-Stereo & seen & $.2691\rightarrow\mathbf{.2623}$ & $1.60\rightarrow\mathbf{1.43}$ & $7.3$ & $8.5$ \\
Stereo Anywhere & seen & $.3022\rightarrow\mathbf{.2779}$ & $2.18\rightarrow\mathbf{1.74}$ & $7.9$ & $13.9$ \\
CREStereo & unseen & $.2668\rightarrow\mathbf{.2583}$ & $1.57\rightarrow\mathbf{1.40}$ & $6.4$ & $11.8$ \\
Fast-FoundationStereo & unseen & $.2725\rightarrow\mathbf{.2658}$ & $1.60\rightarrow\mathbf{1.44}$ & $4.3$ & $9.6$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{The forward--backward cue in training.}
The main paper's runtime paragraph states that training without the
$C^{FB}$ cue costs accuracy and cites this supplement. The number: a head
trained with the channel pinned to $1.0$ (A5, one seed, the locked recipe
otherwise unchanged) reaches $5.31\%$ held-out Bad1 reduction against the
shipped three-seed $5.84\pm0.22$. Seeds 1 and 2 soften the single-run
reading: over three seeds the variant sits at $5.42\pm0.42$ Bad1 and
$5.09\pm0.29$ EPE --- direction consistent, not separable, with a spread
exceeding the registered $0.37$ threshold.
Combined with the two inference-side arms in the main text (pinning costs
the shipped checkpoint $+0.08$; the A5 head fed the real cue moves $-0.02$),
the cue trends useful in training without separating, and is unread at
inference on every checkpoint tested.

\paragraph{The unconstrained-output twin on the shipped base.}
The main paper's ablation section states that A4's $38$-channel twin repeats
the development-overfit pattern and cites this supplement. The numbers (one
seed, no training divergence, best-validation at epoch $7$): development EPE
reduction $11.25\%$ against the shipped $3.69$, collapsing to $4.41$ held out
against our $5.61\pm0.42$; development Bad1 $27.49$ against $12.84$,
falling to $7.32$ held out --- above the shipped $5.84\pm0.22$ on the
registered endpoint, as its $142$-channel parent also is, and declined for
the same reason: the EPE difference is the separable one.

\paragraph{The full metric family of the unified evaluation.}
Spatial geometry: EPE/MAE, median, RMSE, P90/P95/P99, maximum, invalid rate
and Bad-$1/3/5$ on disparity; Bad-$2/5/10$\,mm, AbsRel, SqRel and
$\delta_{1,2,3}$ on metric depth. Temporal fidelity beyond DTCE: given
correspondence, motion-compensated $\mathrm{TEPE}_{\mathrm{corr}}$, with
estimated-flow correspondences marked as proxies; we do not build on this
family because it improves on all five D7 backbones at all four lags but its
two members disagree on D2 beyond one frame. Intervention accounting beyond
the masses and rates: per bad-pixel threshold, NewBad, RecoveredBad, bad-rate
delta and identity preservation --- the fraction of pixels already within
$\tau$ still within it afterwards, a retention rate rather than a count of
untouched pixels, since the blend moves almost every pixel on the support.
Per frame, the mean signed error change and its tails; where a risk score
exists, risk--coverage analysis.
