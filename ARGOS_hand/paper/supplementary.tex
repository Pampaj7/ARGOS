...
BiDAStabilizer \cite{jing2024bida} is the closest plug-in stabiliser in
our comparison set. Its full formulation is non-causal, using
future-frame bidirectional propagation. On the full SCARED-C D2 sequence
set our independent reproduction moves pixel-weighted EPE from
$0.302862$ to $0.302109$.

We deliberately draw no comparison from this. The two evaluation paths
use independently validated but not pixel-identical support contracts,
and the raw EPE differs between them ($0.3029$ against $0.2762$), so the
absolute values are not subtractable. A common-support table is the
correct way to make this comparison and we have not produced one.
