...
class BoundedMemoryPolicy:
    ...
    def pre_reset(self, *, age: int, accumulated_update: float) -> bool:
        ...
        return bool((self.max_age is not None and age >= self.max_age) or (self.accumulated_update_max is not None and accumulated_update > self.accumulated_update_max))

    def evidence_reset(self, evidence: ResetEvidence) -> bool:
        return bool((self.disagreement_max is not None and evidence.disagreement > self.disagreement_max) or (self.warp_support_min is not None and evidence.warp_support < self.warp_support_min) or (self.fb_confidence_min is not None and evidence.fb_confidence < self.fb_confidence_min) or (self.temporal_activation_max is not None and evidence.temporal_activation > self.temporal_activation_max) or (self.update_magnitude_max is not None and evidence.update_magnitude > self.update_magnitude_max))
