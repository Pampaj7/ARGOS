"""Faithful CODD-style bounded fusion head for ARGOS v2."""
...
from .alignment import causal_warp, resize_flow
from .stereo import warp_right_to_left
...
def _patches(value: torch.Tensor, *, window: int = 3, dilation: int = 2) -> torch.Tensor:
    ...
    return F.unfold(value, kernel_size=window, dilation=dilation, padding=dilation).view(batch, channels, window * window, height, width)
...
def _disparity_self_correlation(disparity: torch.Tensor) -> torch.Tensor:
    ...
    return (1.0 - (patch - disparity[:, :, None]).abs().clamp_max(4.0) / 4.0).squeeze(1)
...
def _appearance_self_correlation(feature: torch.Tensor) -> torch.Tensor:
    ...
    return (patch * F.normalize(feature, dim=1, eps=1e-6)[:, :, None]).sum(dim=1)


def _cross_disparity_correlation(current: torch.Tensor, aligned_past: torch.Tensor) -> torch.Tensor:
    return (1.0 - (_patches(aligned_past) - current[:, :, None]).abs().clamp_max(4.0) / 4.0).squeeze(1)
...
def _cross_appearance_correlation(current: torch.Tensor, aligned_past: torch.Tensor) -> torch.Tensor:
    ...
    return (_patches(F.normalize(aligned_past, dim=1, eps=1e-6)) * current[:, :, None]).sum(dim=1)


@torch.no_grad()
def build_codd_cues(extractor: FrozenResNet18Layer1 | None, *, raw: torch.Tensor, aligned_memory: torch.Tensor, current_rgb: torch.Tensor, current_right_rgb: torch.Tensor, past_rgb: torch.Tensor, flow_current_to_past: torch.Tensor, flow_magnitude: torch.Tensor, forward_backward_confidence: torch.Tensor, warp_support: torch.Tensor, aligned_valid: torch.Tensor, include_learned_stereo_evidence: bool = True) -> CODDCues:
    height, width = raw.shape[-2:]
    feature_size = (height // 4, width // 4)
    raw_f, memory_f = _feature_disparity(raw, feature_size), _feature_disparity(aligned_memory, feature_size)
    if include_learned_stereo_evidence:
        if extractor is None:
            raise ValueError("learned stereo evidence requires the frozen extractor")
        current_feature, right_feature, past_feature = extractor(current_rgb), extractor(current_right_rgb), extractor(past_rgb)
        feature_size = current_feature.shape[-2:]
        raw_f, memory_f = _feature_disparity(raw, feature_size), _feature_disparity(aligned_memory, feature_size)
        aligned_past_feature = causal_warp(past_feature, resize_flow(flow_current_to_past, feature_size)).warped
        ...
        low = torch.cat((raw_conf, memory_conf, raw_conf - memory_conf, _disparity_self_correlation(raw_f), _disparity_self_correlation(memory_f), _appearance_self_correlation(current_feature), _appearance_self_correlation(aligned_past_feature), _cross_disparity_correlation(raw_f, memory_f), _cross_appearance_correlation(current_feature, aligned_past_feature), raw_conf_support, memory_conf_support, torch.tanh(current_feature / 3.0)), dim=1)
    else:
        low = torch.cat((_disparity_self_correlation(raw_f), _disparity_self_correlation(memory_f), _cross_disparity_correlation(raw_f, memory_f)), dim=1)
    low = _up(low, (height, width))
    residual = (aligned_memory - raw).clamp(-16, 16) / 16
    motion = torch.cat((flow_current_to_past[:, 0:1].clamp(-32, 32) / 32, flow_current_to_past[:, 1:2].clamp(-32, 32) / 32, flow_magnitude.clamp(0, 32) / 32, forward_backward_confidence.clamp(0, 1), warp_support.float(), aligned_valid.float()), dim=1)
    image = current_rgb.float() / (255.0 if current_rgb.detach().amax() > 1.5 else 1.0)
    values = torch.cat((low, raw.clamp(0, 64) / 64, aligned_memory.clamp(0, 64) / 64, residual, residual.abs(), motion, image * 2.0 - 1.0), dim=1)
    if not torch.isfinite(values).all():
        raise ValueError("non-finite CODD cue")
    return CODDCues(values, warp_support & aligned_valid, values.shape[1])


class _Residual(nn.Module):
    def __init__(self, channels: int) -> None:
        super().__init__()
        self.body = nn.Sequential(nn.Conv2d(channels, channels, 3, padding=1, bias=False), nn.GroupNorm(8, channels), nn.SiLU(inplace=True), nn.Conv2d(channels, channels, 3, padding=1, bias=False), nn.GroupNorm(8, channels))
...
class CODDStyleFusionHead(nn.Module):
    def __init__(self, cue_channels: int, width: int = 48) -> None:
        ...
        self.full = nn.Sequential(nn.Conv2d(cue_channels, width // 2, 3, padding=1, bias=False), nn.GroupNorm(8, width // 2), nn.SiLU(inplace=True), _Residual(width // 2))
        self.coarse = nn.Sequential(nn.Conv2d(width // 2, width, 3, stride=2, padding=1, bias=False), nn.GroupNorm(8, width), nn.SiLU(inplace=True), _Residual(width), _Residual(width), _Residual(width))
        self.reset, self.fusion = nn.Conv2d(width // 2 + width, 1, 3, padding=1), nn.Conv2d(width, 1, 1)
    ...
    def forward(self, cues: CODDCues, raw: torch.Tensor, aligned_memory: torch.Tensor) -> CODDFusionOutput:
        ...
        reset = torch.sigmoid(self.reset(torch.cat((fine, _up(coarse, raw.shape[-2:])), dim=1)))
        fusion = _up(torch.sigmoid(self.fusion(coarse)), raw.shape[-2:])
        temporal = reset * fusion
        return CODDFusionOutput(reset, fusion, temporal, (1.0 - temporal) * raw + temporal * aligned_memory)


def convex_fusion_oracle(raw: torch.Tensor, memory: torch.Tensor, ground_truth: torch.Tensor, *, epsilon: float = 1e-6) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    return (1.0 - weight) * raw + weight * memory, weight


def hard_endpoint_fusion(raw: torch.Tensor, memory: torch.Tensor, temporal_weight: torch.Tensor, threshold: float) -> tuple[torch.Tensor, torch.Tensor]:
