#!/usr/bin/env python3
"""One point of the unlearned residual-rule sweep, scored exactly like the closure baselines.
...
from model_design.comparison.residual_policy import factory as residual_factory
...
def run(config: argparse.Namespace) -> None:
    ...
    output = config.output or RESULTS / f"d2_residual_a{config.alpha:g}_t{config.tau:g}"
    ...
    manifest = {"project": "ARGOS v2", "status": "INCOMPLETE", "dataset": "scared-d2",
                "CUDA_VISIBLE_DEVICES": gpu, "alpha": config.alpha, "tau": config.tau,
    ...
    try:
        for name in config.methods:
            adapter = residual_factory(alpha=config.alpha, tau=config.tau,
            ...
            def save(bundle: Mapping[str, Any], _name: str = name) -> None:
                ...
                atomic_json(output / "reports" / _name / bundle["backbone"] / f"{bundle['sequence_id']}.json",
                            report | {"method": _name, "alpha": config.alpha, "tau": config.tau})
...
def main() -> None:
    ...
    if not config.smoke and (tuple(config.backbones) != tuple(ALL_BACKBONES)
        ...
        raise SystemExit("the sweep still runs every backbone and sequence; pass --smoke to explore")
    run(config)
