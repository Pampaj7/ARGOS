...
if [[ -e "$OUT" ]]; then
  FAILED="$ROOT/results/h4_scared_drends_augmented/failed_attempt_$(date -u +%Y%m%dT%H%M%SZ)_$$"
  mv "$OUT" "$FAILED"
  printf '{"state":"failed","reason":"previous launch exited before completion; archived before relaunch"}\n' >"$FAILED/relaunch_metadata.json"
fi
mkdir -p "$OUT"
...
sha256sum "$RUNNER" "$ROOT/model_design/data/temporal_pair_dataset.py" "$ROOT/model_design/data/drends_temporal_dataset.py" "$ROOT/scripts/argos_v2/paths.py" "$ROOT/model_design/comparison/h4_scared_drends_augmented.py" "$ROOT/model_design/checkpoints/codd_style_h4_best_validation.pt" >"$OUT/source_snapshot.sha256"
cd "$ROOT"
