...
H4_AUGMENTED_TRAIN = TRAIN + ("dataset_2_keyframe_2", "dataset_2_keyframe_3")
H4_AUGMENTED_VALIDATION = ("dataset_2_keyframe_4",)
H4_AUGMENTED_TARGET = ROOT / "model_design/checkpoints/h4_augmented"
H4_CANONICAL_CHECKPOINT = ROOT / "model_design/checkpoints/codd_style_h4_best_validation.pt"
...
def args() -> argparse.Namespace:
    ...
    parser.add_argument("--profile", choices=("legacy", "h4_augmented"), default="legacy")
    ...
    parser.add_argument("--patience", type=int, default=0, help="consecutive non-improving validation epochs; 0 disables early stopping")
    ...
    parser.add_argument("--dry-run", action="store_true", help="validate the selected profile without opening datasets or writing output")
    return parser.parse_args()


def profile_for(config: argparse.Namespace) -> str:
    return getattr(config, "profile", "legacy")


def patience_for(config: argparse.Namespace) -> int:
    return getattr(config, "patience", 0)


def split_for(config: argparse.Namespace) -> tuple[tuple[str, ...], tuple[str, ...], tuple[str, ...]]:
    if profile_for(config) == "h4_augmented":
        return H4_AUGMENTED_TRAIN, H4_AUGMENTED_VALIDATION, TEST
    return TRAIN, VALIDATION, TEST


def config_identity(config: argparse.Namespace) -> dict:
    return {key: str(value) if isinstance(value, Path) else value for key, value in vars(config).items()
            if key not in {"mode", "output", "resume", "dry_run"}}


def resume_compatible(state: dict, split: dict, config: argparse.Namespace) -> bool:
    return state.get("split") == split and state.get("config_identity") == config_identity(config)


def should_stop(patience: int, bad_epochs: int) -> bool:
    return patience > 0 and bad_epochs >= patience


def validate_config(config: argparse.Namespace) -> None:
    if patience_for(config) < 0:
        raise ValueError("patience must be non-negative")
    train_sequences, validation_sequences, test_sequences = split_for(config)
    if set(train_sequences) & set(validation_sequences) or set(train_sequences) & set(test_sequences) or set(validation_sequences) & set(test_sequences):
        raise RuntimeError("train, validation, and test sequence sets must be pairwise disjoint")
    if profile_for(config) != "h4_augmented":
        return
    required = {
        "mode": "train", "seed": 0, "epochs": 150, "batch_size": 32, "workers": 20,
        "preload_workers": 20, "learning_rate": 2e-4, "weight_decay": 1e-4,
        "clip_length": 4, "patience": 10, "coverage_threshold": .50,
        "tau_reset_native_px": 5.0, "tau_fusion_native_px": 1.0, "alpha_reg": .2,
        "memory_state": "recurrent", "disable_learned_stereo_evidence": False,
    }
    incorrect = {key: (getattr(config, key, 0 if key == "patience" else None), value) for key, value in required.items() if getattr(config, key, 0 if key == "patience" else None) != value}
    if incorrect:
        raise ValueError(f"h4_augmented locked configuration mismatch: {incorrect}")
    if config.max_clips_per_sequence is not None:
        raise ValueError("h4_augmented requires all clips (no max-clips-per-sequence)")


def write_status(config: argparse.Namespace, state: str, **extra: object) -> None:
    save_json(config.output / "status.json", {"state": state, "pid": os.getpid(), "updated_unix": time.time(), **extra})
...
def manifest(config: argparse.Namespace) -> dict:
    train_sequences, validation_sequences, test_sequences = split_for(config)
    train_ids = {int(name.split("_")[1]) for name in train_sequences}
    validation_ids = {int(name.split("_")[1]) for name in validation_sequences}
    pairwise_disjoint = not (set(train_sequences) & set(validation_sequences) or set(train_sequences) & set(test_sequences) or set(validation_sequences) & set(test_sequences))
    return {
        "profile": profile_for(config),
        "protocol": ("sequence-disjoint augmented CODD-style fixed-BiDA H4; dataset 2 is shared across train/validation by sequence"
                     if profile_for(config) == "h4_augmented" else "strict dataset-ID-disjoint CODD-style fixed-BiDA phase 1"),
        "train_sequences": list(train_sequences), "validation_sequences": list(validation_sequences), "test_sequences": list(test_sequences),
        "train_dataset_ids": sorted(train_ids), "validation_dataset_ids": sorted(validation_ids), "test_dataset_ids": [7],
        "sequence_sets_pairwise_disjoint": pairwise_disjoint,
        "dataset_id_disjoint": not bool(train_ids & validation_ids),
        "dataset_id_note": ("train and validation deliberately share dataset ID 2 but use disjoint sequences; D7 remains a disjoint unopened test split during training"
                            if profile_for(config) == "h4_augmented" else "dataset IDs are disjoint"),
...
def train(config: argparse.Namespace, *, tiny: bool = False, overfit: bool = False) -> None:
    ...
    train_split, validation_split, _ = split_for(config)
    train_sequences = ("dataset_3_keyframe_1",) if tiny else train_split
    validation_sequences = train_sequences if overfit else (("dataset_2_keyframe_2",) if tiny else validation_split)
    write_status(config, "preloading", train_sequences=list(train_sequences), validation_sequences=list(validation_sequences))
    ...
    write_status(config, "training", preloading_complete=True)
    ...
    history, best, bad_epochs, start_epoch, optimizer, scaler = [], math.inf, 0, 0, None, torch.amp.GradScaler("cuda", enabled=device.type == "cuda")
    last = config.output / "checkpoints/last.pt"
    if config.resume and last.exists() and not tiny:
        state = torch.load(last, map_location="cpu", weights_only=False)
        if profile_for(config) == "h4_augmented" and not resume_compatible(state, split, config):
            raise RuntimeError("refusing h4_augmented resume with incompatible split, configuration, or seed")
        ...
        start_epoch, best, bad_epochs = int(state["epoch"]), float(state["best"]), int(state.get("bad_epochs", 0))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for cpu in loader:
            ...
            if batches == 1 or batches % 10 == 0:
                print(json.dumps({"event": "train_batch", "epoch": epoch + 1, "batch": batches, "loss": float(loss.detach())}), flush=True)
        ...
        current = row["validation_fused_epe"]
        if not math.isfinite(current): raise RuntimeError("validation_fused_epe is not finite")
        improved = current < best
        if improved: best, bad_epochs = current, 0
        else: bad_epochs += 1
        payload = {"model": model.state_dict(), "optimizer": optimizer.state_dict(), "scaler": scaler.state_dict(), "epoch": epoch + 1,
                   "best": best, "bad_epochs": bad_epochs, "cue_channels": model.full[0].in_channels, "config": vars(config),
                   "config_identity": config_identity(config), "split": split, "resnet_sha256": sha256(RESNET18_CHECKPOINT)}
        atomic(last, payload)
        if improved: atomic(config.output / "checkpoints/best_validation.pt", payload)
        print(json.dumps(row), flush=True)
        write_status(config, "training", epoch=epoch + 1, batches=batches, validation_fused_epe=current, best_validation_fused_epe=best, bad_epochs=bad_epochs)
        if should_stop(patience_for(config), bad_epochs):
            print(json.dumps({"event": "early_stop", "epoch": epoch + 1, "bad_epochs": bad_epochs, "patience": patience_for(config)}), flush=True)
            break
    ...
    if profile_for(config) == "h4_augmented" and not tiny:
        promote_augmented(config, split)
    write_status(config, "complete", best_validation_fused_epe=best, epochs_attempted=len(history), early_stopped=should_stop(patience_for(config), bad_epochs))
...
def atomic_copy(source: Path, destination: Path) -> None:
    if destination.exists():
        raise FileExistsError(f"refusing to overwrite final artifact: {destination}")
...
def snapshot_provenance(config: argparse.Namespace, split: dict) -> dict:
    runner = Path(__file__).resolve()
    commit = subprocess.run(["git", "-C", str(ROOT.parent), "rev-parse", "HEAD"], check=False, capture_output=True, text=True).stdout.strip()
    return {
        "profile": profile_for(config), "git_commit": commit or "unavailable",
        "runner": str(runner), "runner_sha256": sha256(runner),
        "canonical_h4_checkpoint": str(H4_CANONICAL_CHECKPOINT), "canonical_h4_checkpoint_sha256": sha256(H4_CANONICAL_CHECKPOINT),
        "config": vars(config), "split": split,
    }


def promote_augmented(config: argparse.Namespace, split: dict) -> None:
    source = config.output / "checkpoints/best_validation.pt"
    if not source.is_file():
        raise FileNotFoundError(f"cannot promote missing best checkpoint: {source}")
    target = H4_AUGMENTED_TARGET
    final = target / "best_validation.pt"
    destinations = (final, target / "provenance.json", target / "configuration.json", target / "split_audit.json", target / "training_history.csv")
    existing = [path for path in destinations if path.exists()]
    if existing:
        raise FileExistsError(f"refusing to overwrite final artifact set: {existing}")
    atomic_copy(source, final)
    provenance = snapshot_provenance(config, split) | {
        "source_checkpoint": str(source), "source_checkpoint_sha256": sha256(source),
        "final_checkpoint": str(final), "final_checkpoint_sha256": sha256(final),
        "promotion": "successful h4_augmented training only; no canonical artifact overwritten",
    }
    for name, value in (("provenance.json", provenance), ("configuration.json", vars(config)), ("split_audit.json", split)):
        temporary = target / f".{name}.tmp"
        save_json(temporary, value); os.replace(temporary, target / name)
    history = config.output / "training_history.csv"
    if history.is_file(): atomic_copy(history, target / "training_history.csv")
...
def main() -> None:
    ...
    if config.dry_run:
        validate_config(config)
        print(json.dumps({"profile": config.profile, "config": vars(config), "split": manifest(config),
                          "canonical_h4_checkpoint_sha256": sha256(H4_CANONICAL_CHECKPOINT)}, sort_keys=True, default=str), flush=True)
        return
    if config.profile == "h4_augmented": validate_config(config)
