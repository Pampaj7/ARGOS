"""Validated causal BiDA-style target-to-source pull alignment for ARGOS v2."""
...
def causal_warp(source: torch.Tensor, flow_target_to_source: torch.Tensor, *, source_valid: torch.Tensor | None = None,
                mode: Literal["bilinear", "nearest"] = "bilinear", valid_threshold: float = 0.999) -> WarpResult:
    if source.ndim != 4:
    ...
    ys, xs = torch.meshgrid(torch.arange(h, device=source.device, dtype=source.dtype),
    ...
    if source_valid is None:
        ...
        valid_sample = F.grid_sample(source_valid.to(source.dtype), grid, mode=mode, padding_mode="zeros", align_corners=True)
...
def forward_backward_consistency(flow_target_to_source: torch.Tensor, flow_source_to_target: torch.Tensor, *,
                                 absolute_threshold: float = 0.5, relative_threshold: float = 0.01) -> ForwardBackwardResult:
...
def photometric_consistency(target_rgb: torch.Tensor, source_rgb: torch.Tensor, flow_target_to_source: torch.Tensor, *,
                            target_valid: torch.Tensor | None = None, source_valid: torch.Tensor | None = None,
                            robust_scale: float = 0.1) -> PhotometricResult:
...
def temporal_disparity_evidence(current_disparity: torch.Tensor, past_disparity: torch.Tensor,
                                flow_current_to_past: torch.Tensor, flow_past_to_current: torch.Tensor, *,
                                current_valid: torch.Tensor, past_valid: torch.Tensor, current_rgb: torch.Tensor,
                                past_rgb: torch.Tensor, fb_absolute_threshold: float = 0.5,
                                fb_relative_threshold: float = 0.01, photometric_robust_scale: float = 0.1) -> TemporalDisparityEvidence:
    ...
    fb = forward_backward_consistency(flow_current_to_past, flow_past_to_current,
                                      absolute_threshold=fb_absolute_threshold, relative_threshold=fb_relative_threshold)
    photo = photometric_consistency(current_rgb, past_rgb, flow_current_to_past, target_valid=current_valid_b,
                                    source_valid=past_valid, robust_scale=photometric_robust_scale)
    ...
    return TemporalDisparityEvidence(aligned_past_disparity=aligned.warped, warp_support=aligned.support,
        aligned_validity=aligned_validity, forward_backward_error=fb.error,
        forward_backward_confidence=fb.confidence, photometric_residual=photo.robust_normalized_residual,
        absolute_disparity_disagreement=signed.abs() * valid_f,
        signed_disparity_disagreement=signed * valid_f, flow_magnitude=flow_magnitude)
