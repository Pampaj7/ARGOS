...
class MultiAnchorEvidence:
    """Backbone-independent cache-grid evidence.

    Scalar maps have shape ``[B,1,H,W]``. Candidate maps have shape
    ``[B,K,H,W]``. No GT, RGB, backbone ID, or future-frame signal belongs in
    this contract.
    """
...
def build_candidate_features(evidence: MultiAnchorEvidence) -> torch.Tensor:
    """Build the fixed 17-channel feature tensor ``[B,K,C,H,W]``."""
    ...
    maps = (
        raw_k.clamp(0, 64) / 64,
        candidates.clamp(0, 64) / 64,
        residual.clamp(-16, 16) / 16,
        abs_residual.clamp(0, 16) / 16,
        local_mean.clamp(0, 8) / 8,
        local_std.clamp(0, 8) / 8,
        scalar(age.float().clamp(0, 8) / 8),
        evidence.candidate_valid.float(),
        evidence.warp_support.float(),
        evidence.fb_confidence.clamp(0, 1),
        witness.float().expand(-1, count, -1, -1) / max(count, 1),
        median.expand(-1, count, -1, -1).clamp(0, 64) / 64,
        mad.expand(-1, count, -1, -1).clamp(0, 8) / 8,
        (candidates - median).abs().clamp(0, 8) / 8,
        (raw - median).abs().expand(-1, count, -1, -1).clamp(0, 8) / 8,
        agreement_count.float().expand(-1, count, -1, -1) / max(count, 1),
        scalar(provenance.float()),
    )
...
class RawMultiAnchorRefiner(nn.Module):
    """Shared shallow CNN; candidates differ only through evidence and age."""
    ...
    def __init__(self, channels: int = 32, blocks: int = 3, max_delta_px: float = 8.0) -> None:
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(FEATURE_CHANNELS, channels, 3, padding=1, bias=False),
            nn.GroupNorm(8, channels), nn.SiLU(inplace=True),
    ...
    def forward(self, evidence: MultiAnchorEvidence) -> MultiAnchorOutput:
        ...
        return MultiAnchorOutput(
            utility_logit=logit,
            utility_probability=probability,
            predicted_delta=delta,
            fusion_weight=weight,
            selection_score=score.masked_fill(unavailable, -torch.inf),
        )
...
def retrieve_and_fuse(
    ...
    *,
    ...
    """Return output, accepted mask, chosen index and chosen fusion weight."""
    score, chosen = output.selection_score.max(dim=1, keepdim=True)
    gather = chosen
    candidate = torch.gather(evidence.candidates, 1, gather)
    probability = torch.gather(output.utility_probability, 1, gather)
    weight = torch.gather(output.fusion_weight, 1, gather)
    chosen_available = torch.gather(evidence.available, 1, gather)
...
__all__ = [
    "FEATURE_CHANNELS", "PROVENANCE_CORRECTED", "PROVENANCE_RAW", "RAW_ANCHOR_AGES",
    "MultiAnchorEvidence", "MultiAnchorOutput", "RawMultiAnchorRefiner",
