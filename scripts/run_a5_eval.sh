...
if [ "${1:-}" = "--node" ]; then
    ...
    cd "$ROOT" || exit 1
    DEST=/dtu/p1/leopam/ARGOS/ARGOS_hand/results/ablation_eval/a5
    "$PY" model_design/comparison/run_definitive_evaluation.py --datasets scared-d2 scared-d7 \
        --module "model_design.comparison.ablation_h4:factory_a5" --device cuda:0 \
        --output "$DEST" || echo "FAILED a5"
    echo "A5 EVAL DONE"
    exit 0
fi
export ESUB_BYPASS=1 ESUB_QUIET=1
exec bsub -I -q p1i -app h100app -n 4 -R "span[hosts=1] rusage[mem=12GB]" \
     -gpu "num=2:mode=shared" -J argos_a5eval "$SELF --node $*"
