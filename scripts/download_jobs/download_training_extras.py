...
S2M2_DIR = Path("../../external/frame_stereo_repos/s2m2/weights/pretrain_weights")
ENDOSLAM_DIR = Path("../../datasets/EndoSLAM")
...
def wait_for_scared():
    ...
    absent_checks = 0
    scared_patterns = ["download_scared_full.py", "download_scared_aria2.sh", "aria2c"]

    def scared_running():
        return any(process_running(pattern) for pattern in scared_patterns)

    while scared_running():
        absent_checks = 0
        time.sleep(60)
    while absent_checks < 5:
        time.sleep(60)
        if scared_running():
            absent_checks = 0
            while scared_running():
