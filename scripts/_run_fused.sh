...
if [ "${1:-}" = "--node" ]; then
    ...
    cd "$ROOT" || exit 1
    exec "$PY" scripts/_dump_fused_window.py --device cuda:0 "$@"
fi
export ESUB_BYPASS=1 ESUB_QUIET=1
exec bsub -I -q p1i -app h100app -n 4 -R "span[hosts=1] rusage[mem=40GB]" -gpu "num=2:mode=shared" -J argos_qual "$SELF --node $*"
