#!/usr/bin/env python3
"""Train EGBM-v2: teacher-trust EGBM refinement.
...
Controlled change over EGBM-v1: same staged recipe, slightly wider model, plus an
explicit trust head. Trust is supervised to open on oracle-beneficial hard positives and
close on hard negatives/raw-good regions. No S2M2/SAV/RAFT/DINO inference.
"""
...
from egbm_v2_refiner import egbm_v2_refiner  # noqa: E402


DEFAULT_ORACLE_TARGETS = Path("results/03_temporal_refinement/evaluation/distillation_targets_selected_clips")
DEFAULT_OUTPUT_ROOT = Path("results/03_temporal_refinement/training/egbm_v2_experimental")
BASELINES = {
...
def residual_loss_batch(model: nn.Module, batch: dict[str, torch.Tensor], args: argparse.Namespace, device: torch.device, source: str) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    trust = diag["trust"].clamp(1e-4, 1.0 - 1e-4)
    trust_target = (sup if "sup" in batch else raw_bad).clamp(0, 1)
    trust_mask = ((sup if "sup" in batch else raw_bad) + raw_good).clamp(0, 1)
    trust_loss = masked_mean(F.binary_cross_entropy(trust, trust_target, reduction="none"), trust_mask) if float(trust_mask.sum()) > 0 else zero
    loss = (
    ...
    return loss, {
        ...
        f"{source}_trust_loss": float(trust_loss.detach().cpu()),
        f"{source}_trust_pos": float(masked_mean(diag["trust"], trust_target * valid).detach().cpu()) if float((trust_target * valid).sum()) > 0 else float("nan"),
        f"{source}_trust_good": float(masked_mean(diag["trust"], raw_good).detach().cpu()) if float(raw_good.sum()) > 0 else float("nan"),
    }
...
def hardneg_loss_batch(model: nn.Module, batch: dict[str, torch.Tensor], args: argparse.Namespace, device: torch.device) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    trust_neg = masked_mean(diag["trust"], hard_neg) if float(hard_neg.sum()) > 0 else zero
    trust_pos = masked_mean(1.0 - diag["trust"], hard_pos) if float(hard_pos.sum()) > 0 else zero
    ...
    loss = (
        ...
        + args.trust_neg_weight * trust_neg
        + args.trust_pos_weight * trust_pos
    ...
    return loss, {
        ...
        "hardneg_trust_neg": float(trust_neg.detach().cpu()), "hardneg_trust_pos": float(trust_pos.detach().cpu()),
        ...
        "trust_hard_neg_mean": float(masked_mean(diag["trust"], hard_neg).detach().cpu()) if float(hard_neg.sum()) > 0 else float("nan"),
        "trust_hard_pos_mean": float(masked_mean(diag["trust"], hard_pos).detach().cpu()) if float(hard_pos.sum()) > 0 else float("nan"),
    }
...
def predict_clip_egbm(model: nn.Module, clip, args: argparse.Namespace, device: torch.device) -> tuple[np.ndarray, np.ndarray, dict[str, np.ndarray]]:
    ...
    refined_all, p_bad_all, damp_all, trust_all = [], [], [], []
    ...
    for s in range(0, n, args.eval_clip_batch):
        ...
        trust_all.append(diag["trust"][:, 0].cpu().numpy())
    return np.concatenate(refined_all), np.concatenate(p_bad_all), {"damping": np.concatenate(damp_all), "trust": np.concatenate(trust_all)}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--trust-weight", type=float, default=0.15)
    p.add_argument("--trust-neg-weight", type=float, default=1.0)
    p.add_argument("--trust-pos-weight", type=float, default=0.5)
...
def main() -> int:
    ...
    core = egbm_v2_refiner(input_channels, args.residual_scale).to(device)
    ...
    run_lines = [
        ...
        f"model=egbm_v2_refiner input_channels={input_channels} residual_scale={args.residual_scale}",
        "design=EGBM-v2 teacher-trust + medium width; same inputs/splits as EGBM-v1",
    ...
    (args.output_root / "README.md").write_text("""# EGBM-v2 Experimental

Design: teacher-trust EGBM with a slightly wider medium backbone.

Rationale: EGBM-v1 already solved most of the safety problem through damping, but its
oracle-gap pressure is indirect. v2 adds one explicit trust valve supervised to open on
oracle-beneficial hard positives and close on hard negatives/raw-good pixels. Inputs,
splits, selected clips, and staged training stay the same as v1. No S2M2/SAV/RAFT/DINO
inference is run.

Success target: beat EGBM-v1 oracle-gap recovery (20.37%) while keeping patho new-Bad3
at or below 1.30%, clean new-Bad3 <=1%, full-GT test <=4.5226 preferred, and runtime
within the 100 ms S2M2+refiner budget.
""")
    ...
    for epoch in range(1, args.stage3_epochs + 1):
        ...
        log(
            ...
            f"trust_hn={metrics.get('trust_hard_neg_mean', float('nan')):.3f} trust_hp={metrics.get('trust_hard_pos_mean', float('nan')):.3f} "
    ...
    # damping/trust analysis on pathological clips
    damping_rows = []
    trust_rows = []
    for clip in patho_clips:
        ...
        trust_rows.append({
            "clip_id": clip.clip_id, "failure_mode": clip.failure_mode,
            "trust_mean_valid": float(trust[valid].mean()),
            "trust_mean_hard_neg": float(trust[hn].mean()) if hn.any() else float("nan"),
            "trust_mean_hard_pos": float(trust[hp].mean()) if hp.any() else float("nan"),
        })
    write_csv(args.output_root / "damping_analysis.csv", damping_rows)
    write_csv(args.output_root / "trust_analysis.csv", trust_rows)
    ...
    summary = {
        "output_root": str(args.output_root), "model": "egbm_v2_refiner", "params": params,
        ...
        "damping_analysis": damping_rows, "trust_analysis": trust_rows, "baselines": BASELINES, "success_criteria": success,
        "no_teacher_inference": True,
    }
    (args.output_root / "aggregate_summary.json").write_text(json.dumps(summary, indent=2, default=str) + "\n")

    (args.output_root / "README.md").write_text(f"""# EGBM-v2 Experimental: Teacher-Trust Staged Training Result

Controlled EGBM-v1 refinement. Same 16-channel input, same splits, same selected clips,
same three-stage recipe. The only architectural change is a wider medium backbone plus
a teacher-trust head that opens on oracle-beneficial hard positives and closes on
hard negatives/raw-good pixels. `refined = raw + residual`; gating, damping, and trust
are already applied internally.
...
Damping/trust sanity (`damping_analysis.csv`, `trust_analysis.csv`): mean valve values
on hard negatives vs hard positives per pathological clip.
""")
