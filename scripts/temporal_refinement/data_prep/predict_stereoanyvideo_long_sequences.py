...
ROOT = ROOT_DIR
SAV_SCRIPT = EXTERNAL_DIR / "video_stereo_repos/stereoanyvideo/scripts/run_argos_scared_smoke.py"
SAV_CKPT = EXTERNAL_DIR / "video_stereo_repos/stereoanyvideo/checkpoints/StereoAnyVideo_MIX.pth"
...
def run_chunk(seq: Path, frame_ids: list[str], temp: Path, out_chunk: Path):
    ...
    subprocess.check_call(cmd, cwd=str(SAV_SCRIPT.parent.parent))
...
def main():
    ...
    for seq in sorted(d for d in args.sequences_root.iterdir() if d.is_dir() and not d.name.startswith("_")):
        ...
        rows = []
        for chunk_idx, i in enumerate(range(0, len(frame_ids), args.chunk_size - args.overlap)):
            chunk_ids = frame_ids[i:i+args.chunk_size]
            if len(chunk_ids) < 20 and len(frame_ids) >= 20:
                chunk_ids = frame_ids[-20:]
