...
DEFAULT_OUTPUT_ROOT = Path("results/03_temporal_refinement/training/egbm_v1_long_control")
BASELINES = {
...
def score_epoch(sel: dict[str, dict[str, float]], fg: dict[str, float]) -> float:
    return (
        sel["all"]["refined_mae"]
        - 0.05 * sel["all"]["oracle_gap_recovered_pct"]
        + 2.0 * max(0.0, sel["patho"]["new_bad3_frame_mean_pct"] - 1.30)
        + 2.0 * max(0.0, sel["clean"]["new_bad3_frame_mean_pct"] - 1.0)
        + 20.0 * max(0.0, fg["refined_mae"] - 4.5226)
    )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--stage1-ckpt", type=Path, default=Path("results/03_temporal_refinement/training/experimental_refiner_vx_training/checkpoints/stage1_detector.pt"))
    p.add_argument("--stage2-epochs", type=int, default=20)
    p.add_argument("--stage3-epochs", type=int, default=24)
    ...
    p.add_argument("--num-workers", type=int, default=48)
    ...
    p.add_argument("--early-stop-patience-stage2", type=int, default=6)
...
def main() -> int:
    ...
    # ---------- Stage 1: Skip and load existing pre-trained detector ----------
    log(f"stage=1 skipping training, loading from {args.stage1_ckpt}")
    ck1 = torch.load(args.stage1_ckpt, map_location=device, weights_only=False)
    unwrap(model).load_state_dict(ck1["model_state_dict"])
    fg = full_gt_eval(model, eval_loaders["val"], device, args.bad_threshold_px)
    save_ckpt(args.output_root / "checkpoints" / "stage1_detector.pt", model, args, splits, 0, 1, {"val_auc": fg["detector_auc"], "val_ap": fg["detector_ap"]})
    stage_summaries["stage1"] = {"loaded_from": str(args.stage1_ckpt), "val_auc": fg["detector_auc"], "val_ap": fg["detector_ap"]}

    # ---------- Stage 2: residual + damping on full-GT ----------
    optimizer = torch.optim.AdamW(core.parameters(), lr=args.lr)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.stage2_epochs, eta_min=args.lr * 0.1)
    best2 = float("inf")
    best2_epoch = 0
    for epoch in range(1, args.stage2_epochs + 1):
        ...
        log(f"stage=2 epoch={epoch} score={score:.4f} val={fg['raw_mae']:.4f}->{fg['refined_mae']:.4f} nb3={fg['new_bad3_from_raw_good_pct']:.3f}% mod={fg['modified_pct']:.2f}% auc={fg['detector_auc']:.4f}")
        if epoch - best2_epoch >= args.early_stop_patience_stage2:
            log(f"early_stop stage=2 epoch={epoch} best_epoch={best2_epoch}")
            break
    ...
    stage_summaries["stage2"] = {"epochs_run": epoch, "best_epoch": ck2["epoch"], "val_metrics": ck2["val_metrics"]}
    ...
    scheduler3 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.stage3_epochs, eta_min=args.stage3_lr * 0.1)
