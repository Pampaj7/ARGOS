#!/usr/bin/env python3
"""Final EGBM-v2 evaluation report.
...
DEFAULT_EGBM_ROOT = Path("results/03_temporal_refinement/training/egbm_v2_experimental")
DEFAULT_OUTPUT = Path("results/03_temporal_refinement/training/egbm_v2_experimental")
...
BASELINE_ROWS = [
    ...
    {
        "method": "EGBM-v1",
        "selected_mae": 10.4032,
        "oracle_gap_recovered_pct": 20.37,
        "patho_new_bad3_pct": 1.30,
        "clean_new_bad3_pct": 0.81,
        "global_frame_mean_new_bad3_pct": 0.96,
        "pixel_weighted_new_bad3_pct": 0.21,
        "modified_pixels_pct": 62.28,
        "full_gt_test_mae": 4.5226,
        "full_gt_test_bad3": 32.941,
        "params": 4042068,
        "runtime_ms_frame": 6.2528,
        "source": "egbm_final_evaluation",
    },
]
...
def load_model(path: Path, device: torch.device) -> tuple[nn.Module, dict[str, Any], argparse.Namespace]:
    ...
    model = egbm_v2_refiner(ckpt.get("input_channels", 16), ckpt_args.residual_scale).to(device)
...
def trust_rows_for_patho(clips: list[Any], pred: dict[str, Any], masks: dict[str, dict[str, np.ndarray]], args: argparse.Namespace) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for clip in [c for c in clips if c.failure_mode in PATHOLOGICAL_MODES]:
        refined, _p_bad, diag = pred[clip.clip_id]
        trust = diag["trust"]
        ...
        for group, mask in groups.items():
            ...
            rows.append({
                ...
                "group": group,
                "pixels": int(mask.sum()),
                "trust_mean": float(vals.mean()) if vals.size else float("nan"),
                "trust_p05": float(np.percentile(vals, 5)) if vals.size else float("nan"),
                "trust_p50": float(np.percentile(vals, 50)) if vals.size else float("nan"),
                "trust_p95": float(np.percentile(vals, 95)) if vals.size else float("nan"),
            })
    return rows
...
def main() -> int:
    ...
    trust_rows = trust_rows_for_patho(clips, pred, masks, stage2_args)
    rows_csv(args.output_root / "trust_analysis.csv", trust_rows)
    ...
    (args.output_root / "benchmark_summary.json").write_text(json.dumps({
        "params": runtime["params"],
        "fp32_ms_per_frame_batched": runtime["fp32_ms_per_frame_batched"],
        "peak_vram_mb": runtime["peak_vram_mb"],
        "estimated_total_ms": runtime["estimated_total_ms"],
    ...
    egbm_row = {
        ...
        "source": "recomputed in benchmark_egbm_v2",
    }
    ...
    beats_v1 = (
        selected_summary["all"]["refined_mae"] < 10.4032
        and selected_summary["all"]["oracle_gap_recovered_pct"] > 20.37
        and selected_summary["pathological"]["new_bad3_frame_mean_pct"] <= 1.30
        and selected_summary["clean"]["new_bad3_frame_mean_pct"] <= 1.0
        and full_test["refined_mae"] <= 4.5226
    )
    verdict = "EGBM-v2 beats v1" if beats_v1 else "EGBM-v2 does not beat v1"
    if strong_breakthrough and beats_v1:
    ...
    readme = f"""# EGBM-v2 Final Evaluation

Verdict: **{verdict}**.

EGBM-v2 is a controlled teacher-trust refinement over EGBM-v1. It keeps the same input
format and staged training, adds a trust head, and uses no teacher inference at eval.

## Key Numbers

| Metric | EGBM-v1 | EGBM-v2 |
|---|---:|---:|
| Selected MAE | 10.4032 | {selected_summary['all']['refined_mae']:.4f} |
| Oracle gap recovered | 20.37% | {selected_summary['all']['oracle_gap_recovered_pct']:.2f}% |
| Pathological new-Bad3 | 1.30% | {selected_summary['pathological']['new_bad3_frame_mean_pct']:.2f}% |
| Clean new-Bad3 | 0.81% | {selected_summary['clean']['new_bad3_frame_mean_pct']:.2f}% |
| Full-GT test MAE | 4.5226 | {full_test['refined_mae']:.4f} |
| Full-GT test Bad-3 | 32.94% | {full_test['refined_bad3']:.3f}% |
| Runtime | 6.25 ms | {runtime['fp32_ms_per_frame_batched']:.3f} ms |

## Interpretation

- Beats EGBM-v1: `{beats_v1}`.
- Beats raw S2M2 on full-GT test: `{full_test['refined_mae'] < full_test['raw_mae']}`, `{full_test['raw_mae']:.4f} -> {full_test['refined_mae']:.4f}`.
- Beats v3.2c on full-GT test: `{full_test['refined_mae'] < 4.6145}`.
...
- Damping/trust are interpretable: see `damping_analysis.csv` and `trust_analysis.csv`.
...
Keep EGBM-v1 as main unless v2 satisfies all hard constraints.
"""
