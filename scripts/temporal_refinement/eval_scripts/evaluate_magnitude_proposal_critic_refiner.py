#!/usr/bin/env python3
"""Evaluate trained MPC checkpoint on selected clips and full-GT val/test."""
...
from train_magnitude_proposal_critic_refiner import selected_eval, full_gt_eval  # noqa: E402


def main() -> int:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--output-root", type=Path, default=Path("results/03_temporal_refinement/training/magnitude_proposal_critic_refiner"))
    ...
    model.eval()
    splits, by_split = load_samples_with_split(cfg.targets_root, cfg.balanced_split_json, cfg.max_frames)
    ...
    eval_loaders = {split: make_loader(ds, cfg.eval_batch_size, max(0, cfg.num_workers // 2), False, cfg.prefetch_factor) for split, ds in full_ds.items()}
    sel, frames, _pred = selected_eval(model, clips, cfg, device)
    fg = {split: full_gt_eval(model, eval_loaders[split], device, cfg.bad_threshold_px) for split in ("val", "test")}
