#!/usr/bin/env python3
"""Benchmark MPC refiner: params, fp32 runtime, VRAM, identity init."""
...
def main() -> int:
    ...
    model = magnitude_proposal_critic_refiner(16, args.residual_scale).to(device).eval()
    params = sum(p.numel() for p in model.parameters())
    x_real, raw = real_batch(args.targets_root, 4, device)
    with torch.no_grad():
        out = model(x_real, args.residual_scale)
    identity_delta = float(out[2].abs().max())
    ...
    row = {"model": "magnitude_proposal_critic_refiner", "params": params, "batch": args.batch, "ms_per_frame_fp32": round(ms, 4), "peak_vram_mb": round(peak, 1), "identity_delta_px": identity_delta}
