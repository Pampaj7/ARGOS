#!/usr/bin/env python3
"""Benchmark EGBM-v3-CARE-S window vs streaming runtime."""
...
from egbm_v3_care_streaming_refiner import egbm_v3_care_streaming, load_v1_warm_start  # noqa: E402


DEFAULT_V1_CHECKPOINT = Path("results/03_temporal_refinement/training/experimental_refiner_vx_training/checkpoints/best.pt")
DEFAULT_OUTPUT = Path("results/03_temporal_refinement/training/egbm_v3_care_streaming")
...
@torch.no_grad()
def time_streaming(model, x, scale, device, warmup=5, iters=30):
    if device.type == "cuda":
        torch.cuda.reset_peak_memory_stats()
        torch.cuda.synchronize()
    for _ in range(warmup):
        state = model.init_state(x.shape[0], x.shape[-2], x.shape[-1], x.device, x.dtype)
        model.forward_step(x, state, scale)
    if device.type == "cuda":
        torch.cuda.synchronize()
    state = model.init_state(x.shape[0], x.shape[-2], x.shape[-1], x.device, x.dtype)
    t0 = time.perf_counter()
    for _ in range(iters):
        _l, _p, _r, _d, state = model.forward_step(x, state, scale)
...
def main() -> int:
    ...
    v2 = egbm_v3_care_streaming(16, args.residual_scale).to(device).eval()
    loaded, total = (0, 0)
    ...
    for name, model in (("egbm_v1", v1), ("egbm_v3_care_s_window", v2)):
        ...
        if name == "egbm_v3_care_s_window":
            ms_s, peak_s = time_streaming(model, x, args.residual_scale, device)
            rows.append({"model": "egbm_v3_care_s_streaming", "params": params, "batch": args.batch, "resolution": "256x320", "precision": "fp32", "ms_per_frame": round(ms_s, 4), "peak_vram_mb": round(peak_s, 1)})
        del x
        if device.type == "cuda":
    ...
    ms_v2 = next(r["ms_per_frame"] for r in rows if r["model"] == "egbm_v3_care_s_streaming")
    summary = {
        "device": str(device),
    ...
    with (args.output_root / "run.log").open("a") as f:
        f.write(f"benchmark: v3_params={sanity['egbm_v3_care_s_window']['params']} streaming_ms={ms_v2} warm={loaded}/{total}\n")
