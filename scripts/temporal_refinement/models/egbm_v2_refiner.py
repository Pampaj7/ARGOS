"""EGBM-v2: teacher-trust Event-Gated Boundary Memory Refiner.
...
v2 keeps the v1 input format and machinery, widens the backbone, and adds one explicit
teacher-trust valve. The trust head is supervised in training: hard positives should
trust oracle-like corrections, hard negatives/raw-good pixels should preserve raw. This
is the smallest controlled change aimed at increasing oracle-gap recovery without
throwing away v1's damping behavior.
...
class EGBMRefiner(nn.Module):
    ...
    def __init__(
        ...
        self.trust_head = nn.Conv2d(c1 + memory_channels, 1, 1)
        ...
        for head in [self.bad_head, self.damping_head, self.trust_head, self.router_head, *self.expert_heads]:
            nn.init.zeros_(head.weight)
            nn.init.zeros_(head.bias)
        nn.init.constant_(self.trust_head.bias, trust_bias)
    ...
    def forward(self, x: torch.Tensor, residual_scale: float | None = None):
        ...
        memory_feat = torch.cat([feat, mem_full], dim=1)
        damping = torch.sigmoid(self.damping_head(memory_feat))
        trust = torch.sigmoid(self.trust_head(memory_feat))
        ...
        for _ in range(self.update_steps):
            r = r + gate * damping * trust * self.update_step_size * (mixture - r)
...
def egbm_v2_refiner(in_channels: int = 16, residual_scale: float = 3.0) -> EGBMRefiner:
    return EGBMRefiner(in_channels, base=96, depths=(2, 3, 5), expansion=2.5, residual_scale=residual_scale)


def egbm_v2_refiner_large(in_channels: int = 16, residual_scale: float = 3.0) -> EGBMRefiner:
    return EGBMRefiner(in_channels, base=112, depths=(2, 3, 6), expansion=2.5, residual_scale=residual_scale)


# Compatibility for reused v1 helpers that import `egbm_refiner` by name.
def egbm_refiner(in_channels: int = 16, residual_scale: float = 3.0) -> EGBMRefiner:
    return egbm_v2_refiner(in_channels, residual_scale)


def egbm_refiner_large(in_channels: int = 16, residual_scale: float = 3.0) -> EGBMRefiner:
    return egbm_v2_refiner_large(in_channels, residual_scale)
