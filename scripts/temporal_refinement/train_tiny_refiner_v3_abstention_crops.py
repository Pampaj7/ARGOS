#!/usr/bin/env python3
"""Train abstaining crop refiner on full S2M2->GT low-res targets."""
...
DEFAULT_OUTPUT_ROOT = Path("results/03_temporal_refinement/training/tiny_refiner_v3_abstention_crops")
...
class AbstentionCropRefiner(nn.Module):
    ...
    def forward(self, x: torch.Tensor, residual_scale: float) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        out = self.net(x)
        bad_logit = out[:, :1]
        residual = residual_scale * torch.tanh(out[:, 1:2])
...
class MetricAcc:
    ...
    def add(self, raw, gt, valid, p_bad, residual, threshold: float) -> None:
        ...
        self.valid += n
        self.raw_abs += float((raw_err * v).sum().detach().cpu())
        self.soft_abs += float((soft_err * v).sum().detach().cpu())
        self.hard_abs += float((hard_err * v).sum().detach().cpu())
        self.raw_bad1 += float(((raw_err > 1.0).float() * v).sum().detach().cpu())
        self.raw_bad3 += float((raw_bad3 * v).sum().detach().cpu())
        self.soft_bad1 += float(((soft_err > 1.0).float() * v).sum().detach().cpu())
        self.soft_bad3 += float(((soft_err > 3.0).float() * v).sum().detach().cpu())
        self.hard_bad1 += float(((hard_err > 1.0).float() * v).sum().detach().cpu())
        self.hard_bad3 += float((hard_bad3 * v).sum().detach().cpu())
        self.raw_good += float(good.sum().detach().cpu())
        self.new_bad3 += float(((raw_err < 1.0).float() * hard_bad3 * v).sum().detach().cpu())
        self.fixed_bad3 += float((raw_bad3 * (hard_err <= 3.0).float() * v).sum().detach().cpu())
        self.degraded_good_abs += float((torch.clamp(hard_err - raw_err, min=0.0) * good).sum().detach().cpu())
        self.modified += float((hard_mask * v).sum().detach().cpu())
        self.corr_abs += float((torch.abs(hard_mask * residual) * v).sum().detach().cpu())
        self.p_good += float((p_bad * good).sum().detach().cpu())
        self.p_mid += float((p_bad * mid).sum().detach().cpu())
        self.p_bad += float((p_bad * bad).sum().detach().cpu())
        self.n_good += float(good.sum().detach().cpu())
        self.n_mid += float(mid.sum().detach().cpu())
        self.n_bad += float(bad.sum().detach().cpu())
...
def loss_batch(model: nn.Module, batch: dict[str, torch.Tensor], args: argparse.Namespace, device: torch.device) -> tuple[torch.Tensor, dict[str, float]]:
...
def train_one_epoch(model: nn.Module, loader: DataLoader, optimizer: torch.optim.Optimizer, args: argparse.Namespace, device: torch.device) -> dict[str, float]:
    model.train()
...
@torch.no_grad()
def evaluate(model: nn.Module, loader: DataLoader, args: argparse.Namespace, device: torch.device, split: str) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    model.eval()
...
def main() -> int:
    ...
    eval_loaders = {split: make_loader(ds, args.eval_batch_size, max(0, args.num_workers // 2), False) for split, ds in full_ds.items()}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_rows.append({"epoch": epoch, **chosen})
        threshold_rows.extend({"epoch": epoch, **r} for r in val_thr_rows)
        if score < best_score:
            best_score = score
            best_epoch = epoch
        ...
        run_lines.append(
            f"epoch={epoch} loss={train_metrics['loss']:.6f} p_bad={train_metrics['p_bad_mean']:.4f} "
    ...
    for split, loader in eval_loaders.items():
        ...
        all_thr_rows.extend({"best_epoch": ckpt["epoch"], **r} for r in rows)
    ...
    planned = args.crops_per_epoch * int(ckpt["epoch"])
    write_csv(
    ...
    (args.output_root / "README.md").write_text(
        f"""# Tiny Refiner v3 Abstention Crops

Crop-trained bad-pixel detector plus residual proposal for full-frame identity fallback.
...
Full-frame evaluation reports soft refinement and hard abstention thresholds in `threshold_sweep.csv`.
