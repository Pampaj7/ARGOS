#!/usr/bin/env python3
"""Train conservative gated tiny refiner on full S2M2->GT low-res targets."""
...
DEFAULT_OUTPUT_ROOT = Path("results/03_temporal_refinement/training/tiny_refiner_v1_1_gated")
...
def batch_metrics(raw, refined, gt, applied, residual, gate, delta, valid, bad_threshold_px: float) -> dict[str, float]:
    raw_err = torch.abs(raw - gt)
    refined_err = torch.abs(refined - gt)
    good = valid * (raw_err < bad_threshold_px).float()
...
def loss_batch(model, batch, args, device) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    applied, residual, gate = model(x, args.residual_scale)
    refined = raw + applied
    raw_err = torch.abs(raw - gt)
    good = valid * (raw_err < args.bad_threshold_px).float()
    ...
    loss = primary + 0.25 * residual_target + args.gate_l1_weight * gate_l1 + args.good_pixel_preserve_weight * preserve + 0.25 * bad_loss
    metrics = {
...
def evaluate(model, loader, args, device, split: str) -> tuple[dict[str, float], list[dict[str, Any]]]:
    ...
    for batch in loader:
        ...
        applied, residual, gate = model(x, args.residual_scale)
        refined = raw + applied
        metrics = batch_metrics(raw, refined, gt, applied, residual, gate, delta, valid, args.bad_threshold_px)
        rows.append(metrics)
        for seq in set(batch["sequence_id"]):
            idx = torch.tensor([s == seq for s in batch["sequence_id"]], device=device, dtype=torch.bool)
            sm = batch_metrics(raw[idx], refined[idx], gt[idx], applied[idx], residual[idx], gate[idx], delta[idx], valid[idx], args.bad_threshold_px)
...
def write_diagnostics(model, dataset: GatedDataset, out_dir: Path, args, device, prefix: str, count: int = 4) -> None:
    ...
    for idx in np.linspace(0, max(0, len(dataset) - 1), min(count, len(dataset)), dtype=int):
        ...
        applied, residual, gate = model(item["x"][None].to(device), args.residual_scale)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--good-pixel-preserve-weight", type=float, default=0.05)
    p.add_argument("--bad-threshold-px", type=float, default=1.0)
...
def main() -> int:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        score = val_metrics["refined_disp_mae_vs_gt"] + max(0.0, val_metrics["refined_bad_3px"] - val_metrics["raw_bad_3px"]) * 0.02
        if score < best:
            best = score
            torch.save(
                {
        ...
        run_log.append(
            ...
            f"gate={val_metrics['gate_mean']:.4f}/{val_metrics['gate_raw_good_mean']:.4f}/{val_metrics['gate_raw_bad_mean']:.4f}"
        )
        (args.output_root / "run.log").write_text("\n".join(run_log) + "\n")
